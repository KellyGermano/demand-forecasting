...
pytestmark = pytest.mark.filterwarnings("ignore:errors='ignore' is deprecated:FutureWarning")
...
class TestTimeConversionFormats:
    ...
    def test_to_datetime_format_YYYYMMDD_with_nat(self, cache):
        ...
        with pytest.raises(
            ValueError,
            match=(
                'unconverted data remains when parsing with format "%Y%m%d": ".0", ' "at position 0"
            ),
        ):
...
class TestToDatetime:
    ...
    def test_to_datetime_array_of_dt64s(self, cache, unit):
        ...
        if not cache:
            ...
            expected = DatetimeIndex(
                [Timestamp(dts_with_oob[0]).asm8, Timestamp(dts_with_oob[1]).asm8] * 30 + [NaT],
            )
        else:
    ...
    def test_to_datetime_tz_pytz(self, cache):
        ...
        arr = np.array(
            [
                us_eastern.localize(datetime(year=2000, month=1, day=1, hour=3, minute=0)),
    ...
    def test_datetime_invalid_index(self, values, format):
        ...
        tm.assert_index_equal(res, Index(values, dtype="object" if format is None else "str"))
...
class TestToDatetimeUnit:
    ...
    def test_to_datetime_unit_with_nulls(self, null):
        ...
        result = to_datetime(ser, unit="s")
        expected = Series(
            [Timestamp("2013-06-09 02:42:28") + timedelta(seconds=t) for t in range(20)] + [NaT],
            dtype="M8[ns]",
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_to_datetime_unit_fractional_seconds(self):
        ...
        result = to_datetime(ser, unit="s")
        expected = Series(
            [Timestamp("2013-06-09 02:42:28") + timedelta(seconds=t) for t in np.arange(0, 2, 0.25)]
            + [NaT],
            dtype="M8[ns]",
        )
...
class TestToDatetimeMisc:
    ...
    def test_to_datetime_iso8601_fails(self, input, format, exact):
        ...
        with pytest.raises(
            ValueError,
            match=(rf"time data \"{input}\" doesn't match format " rf"\"{format}\", at position 0"),
        ):
    ...
    def test_to_datetime_iso8601_separator(self, input, format):
        ...
        with pytest.raises(
            ValueError,
            match=(
                rf"time data \"{input}\" doesn\'t match format " rf"\"{format}\", at position 0"
            ),
        ):
            to_datetime(input, format=format)
...
class TestShouldCache:
    ...
    def test_should_cache(self, listlike, do_caching):
        assert (
            tools.should_cache(listlike, check_count=len(listlike), unique_share=0.7) == do_caching
        )
