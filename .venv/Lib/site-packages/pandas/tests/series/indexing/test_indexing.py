...
def test_timedelta_assignment():
    ...
    expected = Series([np.nan, Timedelta("1 days")], dtype="timedelta64[ns]", index=["A", "B"])
    tm.assert_series_equal(s, expected)
...
def test_multilevel_preserve_name(lexsorted_two_level_string_multiindex, indexer_sl):
    ...
    ser = Series(np.random.default_rng(2).standard_normal(len(index)), index=index, name="sth")
