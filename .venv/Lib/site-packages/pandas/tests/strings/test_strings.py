...
def test_repeat(any_string_dtype):
    ...
    expected = Series(["aaa", "bbb", np.nan, "ccc", np.nan, "ddd"], dtype=any_string_dtype)
    tm.assert_series_equal(result, expected)
...
def test_isnumeric_unicode(method, expected, any_string_dtype):
    ...
    if any_string_dtype == "object" or (
        isinstance(any_string_dtype, StringDtype) and any_string_dtype.storage == "python"
    ):
...
def test_len(any_string_dtype):
    ...
    result = ser.str.len()
    expected_dtype = "float64" if is_object_or_nan_string_dtype(any_string_dtype) else "Int64"
