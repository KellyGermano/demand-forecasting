...
class MultiColumnCompletionMenuControl(UIControl):
    ...
    def preferred_width(self, max_available_width: int) -> int | None:
        ...
        result = int(
            column_width * math.ceil(len(complete_state.completions) / float(self.min_rows))
        )
    ...
    def create_content(self, width: int, height: int) -> UIContent:
        ...
        for row_index, row in enumerate(rows_):
            ...
            # Add line.
            fragments_for_line.append(to_formatted_text(fragments, style="class:completion-menu"))
    ...
    def _get_column_width(self, completion_state: CompletionState) -> int:
        ...
        try:
            ...
            result = max(get_cwidth(c.display_text) for c in completion_state.completions) + 1
...
class _SelectedCompletionMetaControl(UIControl):
    ...
    def preferred_width(self, max_available_width: int) -> int | None:
        ...
        if app.current_buffer.complete_state:
            ...
            return 2 + max(get_cwidth(c.display_meta_text) for c in state.completions[:100])
        else:
            return 0
