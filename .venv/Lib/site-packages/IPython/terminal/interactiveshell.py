...
class TerminalInteractiveShell(InteractiveShell):
    ...
    def _highlighting_style_changed(self, change):
        ...
        if change.new != "legacy":
            warn("highlighting_style is deprecated since 9.0 and have no effect, use themeing.")
            return
    ...
    editor = Unicode(
        get_default_editor(), help="Set the editor used by IPython (default to $EDITOR/vi/notepad)."
    ).tag(config=True)
    ...
    def _merge_shortcuts(self, user_shortcuts):
        ...
        allowed_commands = {
            create_identifier(binding.command): binding.command for binding in KEY_BINDINGS
        }
        allowed_commands.update(
            {create_identifier(command): command for command in UNASSIGNED_ALLOWED_COMMANDS}
        )
        shortcuts_to_skip = []
        shortcuts_to_add = []
        ...
        for shortcut in user_shortcuts:
            ...
            old_filter = (
                filter_from_string(shortcut["match_filter"]) if "match_filter" in shortcut else None
            )
            matching = [
                binding
            ...
            if modifying_existing:
                specification = {
                    key: shortcut[key] for key in ["command", "filter"] if key in shortcut
                }
                if len(matching) == 0:
                    raise ValueError(
    ...
    def init_display_formatter(self):
        ...
        if not ("DisplayFormatter" in config and "active_types" in config["DisplayFormatter"]):
        ...
        with patch_stdout(raw=True):
            if self._use_asyncio_inputhook:
                ...
                text = asyncio_loop.run_until_complete(
                    self.pt_app.prompt_async(default=default, **self._extra_prompt_options())
                )
            else:
