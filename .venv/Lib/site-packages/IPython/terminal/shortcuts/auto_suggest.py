    ...
    def apply_transformation(self, ti: TransformationInput) -> Transformation:
        ...
        if is_last_line:
            if delta < len(suggestions):
                suggestion = (
                    f"… rest of suggestion ({len(suggestions) - delta} lines) and code hidden"
                )
...
class NavigableAutoSuggestFromHistory(AutoSuggestFromHistory):
    ...
    def _cancel_running_llm_task(self) -> None:
        ...
        if self._llm_task is not None:
            ...
            if cancelled:
                self._llm_task = None
            if not cancelled:
                warnings.warn("LLM task not cancelled, does your provider support cancellation?")
    ...
    async def _trigger_llm_core(self, buffer: Buffer):
        ...
        async for reply_and_chunks in self._llm_provider.stream_inline_completions(request):
            ...
            if isinstance(reply_and_chunks, jai_models.InlineCompletionReply):
                ...
                buffer.suggestion = Suggestion(reply_and_chunks.list.items[0].insertText)
