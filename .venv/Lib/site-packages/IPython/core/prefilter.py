...
class PrefilterManager(Configurable):
    ...
    def prefilter_lines(self, lines, continue_prompt=False):
        ...
        if len(llines) > 1:
            out = "\n".join(
                [self.prefilter_line(line, lnum > 0) for lnum, line in enumerate(llines)]
            )
        else:
...
class AutocallChecker(PrefilterChecker):
    ...
    exclude_regexp = CRegExp(
        re_exclude_auto, help="RegExp to exclude strings with this start from autocalling."
    ).tag(config=True)
    ...
    def check(self, line_info):
        ...
        if (
            ...
            and (line_info.raw_the_rest.startswith(" ") or not line_info.raw_the_rest.strip())
        ):
            return self.prefilter_manager.get_handler_by_name("auto")
        else:
            return None
