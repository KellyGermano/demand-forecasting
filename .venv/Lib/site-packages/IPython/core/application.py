...
class BaseIPythonApplication(Application):
    ...
    overwrite = Bool(False, help="""Whether to overwrite existing config files when copying""").tag(
        config=True
    )

    auto_create = Bool(False, help="""Whether to create profile dir if it doesn't exist""").tag(
        config=True
    )
        ...
        for config_file_name in self.config_files:
            ...
            try:
                ...
                if not suppress_errors:
                    raise
                self.log.warning(
                    "Error loading config file: %s" % self.config_file_name, exc_info=True
                )
