...
class BasicMagics(Magics):
    ...
    @magic_arguments.argument(
        "-p", "--params", default=None, help="""Parameters passed to the magic function."""
    )
    @line_magic
    def alias_magic(self, line=""):
        ...
        if args.line and m_line is None:
            raise UsageError(
                "Line magic function `%s%s` not found." % (magic_escapes["line"], target)
            )
        if args.cell and m_cell is None:
            raise UsageError(
                "Cell magic function `%s%s` not found." % (magic_escapes["cell"], target)
            )
        ...
        if args.line:
            ...
            print(
                ...
                % (magic_escapes["line"], name, magic_escapes["line"], target, params_str)
            )
        ...
        if args.cell:
            ...
            print(
                "Created `%s%s` as an alias for `%s%s%s`."
                % (magic_escapes["cell"], name, magic_escapes["cell"], target, params_str)
            )
    ...
    def notebook(self, s):
        ...
        for session, execution_count, source in hist[:-1]:
            ...
            if execution_count in exceptions:
                cell.outputs.append(v4.new_output("error", **exceptions[execution_count]))
            cells.append(cell)
