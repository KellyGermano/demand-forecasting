...
class InteractiveShell(SingletonConfigurable):
    ...
    banner1 = Unicode(
        default_banner, help="""The part of the banner to be printed before the profile"""
    ).tag(config=True)
    banner2 = Unicode("", help="""The part of the banner to be printed after the profile""").tag(
        config=True
    )
    ...
    show_rewritten_input = Bool(True, help="Show rewritten input, e.g. for autocall.").tag(
        config=True
    )
    ...
    call_pdb = property(
        _get_call_pdb, _set_call_pdb, None, "Control auto-activation of pdb at exceptions"
    )
    ...
    def prepare_user_module(self, user_module=None, user_ns=None):
        ...
        if user_module is None:
            user_module = types.ModuleType(
                "__main__", doc="Automatically created module for IPython interactive environment"
            )
    ...
    def register_magic_function(self, func, magic_kind="line", magic_name=None):
        self.magics_manager.register_function(func, magic_kind=magic_kind, magic_name=magic_name)
        ...
        if fn is None:
            ...
            extra = (
                ...
                if cm is None
                    ...
                    " (But cell magic `%%%%%s` exists, " "did you mean that instead?)" % magic_name
                )
            )
    ...
    def _tee(self, channel: Literal["stdout", "stderr"]):
        ...
        def write(data, *args, **kwargs):
            ...
            if output_stream is None:
                output_stream = HistoryOutput(output_type=output_type, bundle={"stream": []})
    ...
    def run_cell(
        ...
        with self._tee(channel="stdout"), self._tee(channel="stderr"):
            try:
                result = self._run_cell(raw_cell, store_history, silent, shell_futures, cell_id)
            finally:
        ...
        *,
        ...
        if store_history:
            ...
            if result.error_in_exec:
                # Store formatted traceback and error details
                self.history_manager.exceptions[exec_count] = self._format_exception_for_storage(
        ...
        try:
            ...
            for node, mode in to_run:
                ...
                with compiler.extra_flags(
                    getattr(ast, "PyCF_ALLOW_TOP_LEVEL_AWAIT", 0x0) if self.autoawait else 0x0
                ):
    ...
    def find_user_code(
        ...
        try:  # User namespace
            ...
            raise ValueError(
                ("'%s' was not found in history, as a file, url, " "nor in the user namespace.")
                % target
            ) from e
