...
class HistoryTrim(BaseIPythonApplication):
    ...
    backup = Bool(False, help="Keep the old history file as history.sqlite.<N>").tag(config=True)

    keep = Int(1000, help="Number of recent lines to keep in the database.").tag(config=True)

    flags = Dict(dict(backup=({"HistoryTrim": {"backup": True}}, backup.help)))  # type: ignore
    ...
    def start(self):
        ...
        if inputs:
            ...
            outputs = list(
                con.execute(
                    "SELECT session, line, output FROM " "output_history WHERE session >= ?",
                    (first_session,),
                )
            )
            sessions = list(
