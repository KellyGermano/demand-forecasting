...
def _coerce_path_to_tuples(
    ...
    return {path if isinstance(path, tuple) else tuple(path.split(".")) for path in allow_list}
...
class SelectivePolicy(EvaluationPolicy):
    ...
    _operation_methods_cache: dict[str, set[Callable]] = field(default_factory=dict, init=False)
    ...
    def can_operate(self, dunders: tuple[str, ...], a, b=None):
        allowed_operations_external = _coerce_path_to_tuples(self.allowed_operations_external)
        objects = [a]
        if b is not None:
            objects.append(b)
        return all(
...
def _validate_policy_overrides(policy_name: EvaluationPolicyName, policy_overrides: dict) -> bool:
    policy = EVALUATION_POLICIES[policy_name]

    all_good = True
    for key, value in policy_overrides.items():
        if not hasattr(policy, key):
            warnings.warn(f"Override {key!r} is not valid with {policy_name!r} evaluation policy")
            all_good = False
    return all_good
    ...
    for target in node.targets:
        if isinstance(target, (ast.Tuple, ast.List)):
            ...
            if starred:
                ...
                transient_locals[targets[star_or_last_idx].value.id] = values[star_or_last_idx:end]

                # After starred
                for i in range(star_or_last_idx + 1, len(targets)):
                    transient_locals[targets[i].id] = values[len(values) - (len(targets) - i)]
        else:
...
def _extract_args_and_kwargs(node: ast.Call, context: EvaluationContext):
    ...
    kwargs = {
        ...
        for k, v in (
            {kw.arg: eval_node(kw.value, context)} if kw.arg else eval_node(kw.value, context)
        ).items()
    }
    return args, kwargs
...
def eval_node(node: Union[ast.AST, None], context: EvaluationContext):
    ...
    if isinstance(node, ast.FunctionDef):
        ...
        if is_property:
            context.transient_locals[node.name] = _resolve_annotation(return_type, context)
            return None
    ...
    if isinstance(node, ast.Compare):
        ...
        for op, right in zip(node.ops, node.comparators):
            ...
            if dunder:
                ...
                raise ValueError(f"Comparison `{dunder}` not supported")  # pragma: no cover
        return all_true
        ...
        if dunders:
            if policy.can_operate(dunders, value):
                try:
                    ...
                    raise TypeError(f"bad operand type for unary {node.op}: {type(value)}")
            else:
                raise GuardRejection(
                    f"Operation (`{dunders}`) for",
                    type(value),
...
def _eval_annotation(
    ...
    return _eval_node_name(annotation, context) if isinstance(annotation, str) else annotation
...
def _resolve_annotation(
    ...
    if annotation is Self and func and hasattr(func, "__self__"):
        ...
        if func and hasattr(func, "__node__"):
            ...
            for i, arg in enumerate(def_node.args.args):
                ...
                if annotation is AnyStr:
                    index = i
                    break
            is_bound_method = isinstance(func, MethodType) and getattr(func, "__self__") is not None
        ...
        attributes = [
            attr
            for attr in dir(_resolve_annotation(get_args(annotation)[0], context, sig, func, node))
        ]
        ...
        return tuple(
            _resolve_annotation(arg, context, sig, func, node) for arg in get_args(annotation)
        )
