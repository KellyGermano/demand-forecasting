...
def has_any_completions(result: MatcherResult) -> bool:
    ...
    if _is_iterator(completions):
        try:
            ...
            return False
    raise ValueError("Completions returned by matcher need to be an Iterator or a Sizable")
...
def rectify_completions(text: str, completions: _IC, *, _debug: bool = False) -> _IC:
    ...
    for c in completions:
        ...
        yield Completion(
            new_start, new_end, new_text, type=c.type, _origin=c._origin, signature=c.signature
        )
...
class Completer(Configurable):
    ...
    def global_matches(self, text: str, context: Optional[CompletionContext] = None):
        ...
        if context and context.full_text.count("\n") > 1:
            ...
            if previous_lines:
                all_code_lines_before_cursor = self._extract_code(previous_lines) + "\n" + text
    ...
    def _strip_code_before_operator(self, code: str) -> str:
        ...
        try:
            ...
            try:
                _ = list(tokenize.generate_tokens(iter(trimmed_code.splitlines()).__next__))
                code = trimmed_code
            except tokenize.TokenError:
                return code
...
def match_dict_keys(
    ...
    filtered_key_is_final: dict[Union[str, bytes, int, float], _DictKeyState] = defaultdict(
        lambda: _DictKeyState.BASELINE
    )
...
class IPCompleter(Completer):
    ...
    def file_matcher(self, context: CompletionContext) -> SimpleMatcherResult:
        ...
        if text == "":
            return {
                "completions": [
                    SimpleCompletion(text=text_prefix + protect_filename(f), type="path")
                    for f in self.glob("*")
                ],
                "suppress": False,
            }
    ...
    def _extract_code(self, line: str) -> str:
        ...
        try:
            if magic_name == "timeit":
                ...
                return stmt
            elif hasattr(magic_method, "parser") and getattr(magic_method, "has_arguments", False):
                # e.g. %debug, %time
    ...
    def magic_matcher(self, context: CompletionContext) -> SimpleMatcherResult:
        ...
        return {
            "completions": [SimpleCompletion(text=comp, type="magic") for comp in completions],
    ...
    def magic_config_matches(self, text: str) -> list[str]:
        ...
        if len(texts) > 0 and (texts[0] == "config" or texts[0] == "%config"):
            ...
            classes = sorted(
                set([c for c in self.shell.configurables if c.__class__.class_traits(config=True)]),
    ...
    def _jedi_matches(
        ...
        try:
            ...
            try:
                first_child = next(
                    c for c in interpreter._get_module().tree_node.children if hasattr(c, "value")
                )
            except StopIteration:
                pass
            else:
        ...
        try:
            return filter(
                completion_filter, interpreter.complete(column=cursor_column, line=cursor_line + 1)
            )
        except Exception as e:
            if self.debug:
                return iter(
    ...
    def _is_in_string_or_comment(self, text):
        ...
        while i < len(text):
            ...
            if (
                ...
                and not (in_single_quote or in_double_quote or in_triple_single or in_triple_double)
            ):
            ...
            if i + 2 < len(text):
                if text[i : i + 3] == '"""' and not in_single_quote and not in_triple_single:
                    in_triple_double = not in_triple_double
                    if not in_triple_double:
                        in_template_string = False
                    i += 3
                    continue
                if text[i : i + 3] == "'''" and not in_double_quote and not in_triple_double:
            ...
            # Check for comment
            if text[i] == "#" and not (in_single_quote or in_double_quote or in_triple_quote):
                return True, False

            i += 1

        is_string = in_single_quote or in_double_quote or in_triple_single or in_triple_double
    ...
    def python_matcher(self, context: CompletionContext) -> SimpleMatcherResult:
        ...
        if completion_type == self._CompletionContextType.ATTRIBUTE:
            try:
                matches, fragment = self._attr_matches(text, include_prefix=False, context=context)
                if text.endswith(".") and self.omit__names:
                    if self.omit__names == 1:
                        ...
                        no__name = lambda txt: re.match(r"\._.*?", txt[txt.rindex(".") :]) is None
            ...
            return SimpleMatcherResult(
                completions=[SimpleCompletion(text=match, type="variable") for match in matches],
                suppress=False,
            )
    ...
    def _complete(
        ...
        matchers = {
            ...
            for matcher in sorted(self.matchers, key=_get_matcher_priority, reverse=True)
        }
