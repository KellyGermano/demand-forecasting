...
class Alias:
    ...
    def validate(self):
        ...
        if self.name in self.blacklist:
            raise InvalidAliasError(
                "The name %s can't be aliased " "because it is a keyword or builtin." % self.name
            )
        try:
        ...
        if not (isinstance(self.cmd, str)):
            raise InvalidAliasError("An alias command must be a string, " "got: %r" % self.cmd)
    ...
    def __call__(self, rest=""):
        ...
        if nargs == 0:
            ...
            if len(args) < nargs:
                raise UsageError(
                    "Alias <%s> requires %s arguments, %s given." % (self.name, nargs, len(args))
                )
