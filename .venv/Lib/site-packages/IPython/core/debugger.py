...
class Pdb(OldPdb):
            ...
            if not arg:
                for ix, exc in enumerate(self._chained_exceptions):
                    ...
                    if len(rep) > 80:
                        rep = rep[:77] + "..."
                    indicator = (
                        "  -" if self._chained_exceptions[ix].__traceback__ is None else f"{ix:>3}"
                    )
    ...
    def __line_content(self, filename: str, lineno: int, line: str, arrow: bool = False):
        bp_mark = ""
        ...
        try:
            ...
            for lineno in range(first, last + 1):
                ...
                if lineno == self.curframe.f_lineno:
                    bp, num, colored_line = self.__line_content(filename, lineno, line, arrow=True)
                    toks.extend(
                        [
                            bp,
                            (Token.LinenoEm, num),
                    ...
                    bp, num, colored_line = self.__line_content(filename, lineno, line, arrow=False)
                    toks.extend(
                        [
                            bp,
                            (Token.Lineno, num),
