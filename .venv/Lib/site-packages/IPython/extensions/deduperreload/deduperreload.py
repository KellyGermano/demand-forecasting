...
class GatherResult(NamedTuple):
    ...
    function_defs: list[tuple[tuple[str, ...], ast.FunctionDef | ast.AsyncFunctionDef]] = []
...
class DeduperReloader(DeduperReloaderPatchingMixin):
    ...
    @classmethod
    def is_constexpr_assign(cls, node: ast.AST, parent_node: ast.Module | ast.ClassDef) -> bool:
    ...
    def detect_autoreload(
        ...
        cur.defs_to_delete |= set(old_defs_by_name.keys()) - set(new_defs_by_name.keys())
    ...
    def _add_node_to_autoreload_tree(self, node: DependencyNode) -> None:
        ...
        if node.abstract_syntax_tree is not None:
            cur.defs_to_reload.append(((node.qualified_name[-1],), node.abstract_syntax_tree))

    def _check_dependents_inner(self, prefixes: list[str] | None = None) -> list[DependencyNode]:
        prefixes = prefixes or []
        cur = self._to_autoreload.traverse_prefixes(prefixes)
        ans = []
        ...
        for names, new_ast_def in cur.defs_to_reload:
            ...
            if (
                ...
                if len(cast(ast.FunctionDef, func_asts[0].body[0]).decorator_list) > 0:
                    ...
                    cast(ast.FunctionDef, without_decorator_list.body[0]).decorator_list = []
                    func_asts.insert(0, without_decorator_list)
                for func_ast in func_asts:
                    compiled_code = compile(func_ast, filename, mode="exec", dont_inherit=True)
    ...
    def _patch_namespace(self, ns: ModuleType | type, prefixes: list[str] | None = None) -> bool:
        """
