...
class Win32ShellCommandController:
    ...
    def __enter__(self):
        ...
        try:
            ...
            def create_pipe(uninherit):
                ...
                if not CreatePipe(
                    ctypes.byref(handles[0]), ctypes.byref(handles[1]), ctypes.byref(saAttr), 0
                ):
    ...
    def run(self, stdout_func=None, stdin_func=None, stderr_func=None):
        ...
        threads.append(
            threading.Thread(target=self._stdout_thread, args=(self.hstdout, stdout_func))
        )
        if not self.mergeout:
            if stderr_func is None:
                stderr_func = stdout_func
            threads.append(
                threading.Thread(target=self._stdout_thread, args=(self.hstderr, stderr_func))
            )
