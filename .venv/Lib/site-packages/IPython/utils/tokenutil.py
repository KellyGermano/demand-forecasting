...
def generate_tokens_catch_errors(readline, extra_errors_to_catch: list[str] | None = None):
...
def token_at_cursor(cell: str, cursor_pos: int = 0) -> str:
    ...
    tokens: list[Token | None] = [Token(*tup) for tup in generate_tokens(StringIO(cell).readline)]
    if not tokens:
        return ""
    for prev_tok, (tok, next_tok) in zip([None] + tokens, itertools.pairwise(tokens + [None])):
        ...
        if cur_token_is_name := tok.token == tokenize.NAME and not iskeyword(tok.text):
            if names and prev_tok and prev_tok.token == tokenize.OP and prev_tok.text == ".":
                names[-1] = "%s.%s" % (names[-1], tok.text)
            else:
                names.append(tok.text)
            if next_tok is not None and next_tok.token == tokenize.OP and next_tok.text == "=":
