...
class Demo:
    ...
    def __init__(
        ...
        self.formatter = pygments.formatters.get_formatter_by_name(formatter, style=style)
    ...
    def show(self, index=None):
        ...
        print(
            self.marquee(
                "<%s> block # %s (%s remaining)" % (self.title, index, self.nblocks - index - 1)
            )
        )
    ...
    def show_all(self):
        ...
        for index, block in enumerate(self.src_blocks_colored):
            if silent[index]:
                ...
                print(
                    marquee("<%s> block # %s (%s remaining)" % (title, index, nblocks - index - 1))
                )
            print(block, end=" ")
        sys.stdout.flush()
...
def slide(
    ...
    demo = demo_class(
        file_path, format_rst=format_rst, formatter=formatter, style=style, auto_all=auto_all
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--noclear", "-C", action="store_true", help="Do not clear terminal on each slide"
    )
    parser.add_argument(
        "--rst", "-r", action="store_true", help="Highlight comments and dostrings as rst"
    )
    parser.add_argument(
        "--formatter",
        "-f",
    ...
    parser.add_argument(
        "--delimiter", "-d", default="...", help="slides delimiter added after each slide run"
    )
