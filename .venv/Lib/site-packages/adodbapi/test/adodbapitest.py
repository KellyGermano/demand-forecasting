...
class CommonDBTests(unittest.TestCase):
    ...
    def testUserDefinedConversions(self):
        try:
            ...
            crsr = conn.cursor()
            tabdef = (
                "CREATE TABLE xx_%s (fldData VARCHAR(100) NOT NULL, fld2 VARCHAR(20))" % config.tmp
            )
            crsr.execute(tabdef)
    ...
    def helpTestDataType(
        ...
        for inParam in inputs:
            ...
            if allowedReturnValues:
                ...
                assert isinstance(rs[0], allowedTypes), 'result type "%s" must be one of %s' % (
                    type(rs[0]),
                    allowedTypes,
                )
            else:
            ...
            if compareAlmostEqual and DBAPIDataTypeString == "DATETIME":
                ...
                s = float(pyData)
                v = float(rs[0])
                assert abs(v - s) / s < 0.00001, "Values not almost equal recvd=%s, expected=%f" % (
                    rs[0],
                    s,
                )
            else:
    ...
    def testRowIterator(self):
        ...
        for inParam in inputs:
            ...
            for j in range(len(inParam)):
                assert rec[j] == inParam[j], 'returned value:"%s" != test value:"%s"' % (
                    rec[j],
                    inParam[j],
                )
            # check that we can get a complete tuple from a row
            assert tuple(rec) == inParam, f'returned value:"{rec!r}" != test value:"{inParam!r}"'
            # test that slices of rows work
            slice1 = tuple(rec[:-1])
            slice2 = tuple(inParam[0:2])
            assert slice1 == slice2, f'returned value:"{slice1!r}" != test value:"{slice2!r}"'
    ...
    def testNamedParamstyle(self):
        ...
        for inParam in inputs:
            ...
            try:
                crsr.execute(
                    "INSERT INTO xx_%s (fldId,fldData) VALUES (:Id,:f_Val)" % config.tmp,
    ...
    def testPyformatParamstyle(self):
        ...
        for inParam in inputs:
            ...
            try:
                crsr.execute(
                    "INSERT INTO xx_%s (fldId,fldData) VALUES (%%(Id)s,%%(f_Val)s)" % config.tmp,
                    {"f_Val": inParam, "Id": fldId},
                )
            except:
                conn.printADOerrors()
        ...
        # now a test with a "%" as part of a literal
        crsr.execute(
            "insert into xx_%s (fldId,fldData) VALUES (%%(xyz)s,'six%%five')" % config.tmp,
            {"xyz": 30},
        )
        crsr.execute("SELECT fldData FROM xx_%s WHERE fldID=30" % config.tmp)
        rec = crsr.fetchone()
    ...
    def testAutoRollback(self):
        ...
        try:
            ...
            row = (
                None  # if the entire table disappeared the rollback was perfect and the test passed
            )
        assert (
            row is None
...
class TestADOwithSQLServer(CommonDBTests):
    ...
    def testVariableReturningStoredProcedure(self):
        ...
        assert retvalues[2] == "DodsworthAnne", f'{retvalues[2]!r} is not "DodsworthAnne"'
        self.conn.rollback()
...
suites = [unittest.defaultTestLoader.loadTestsFromModule(TestPythonDateTimeConverter, "test")]
if config.doTimeTest:
    suites.append(unittest.defaultTestLoader.loadTestsFromModule(TestPythonTimeConverter, "test"))
if config.doAccessTest:
    suites.append(unittest.defaultTestLoader.loadTestsFromModule(TestADOwithAccessDB, "test"))
if config.doSqlServerTest:
    suites.append(unittest.defaultTestLoader.loadTestsFromModule(TestADOwithSQLServer, "test"))
if config.doMySqlTest:
    suites.append(unittest.defaultTestLoader.loadTestsFromModule(TestADOwithMySql, "test"))
if config.doPostgresTest:
    suites.append(unittest.defaultTestLoader.loadTestsFromModule(TestADOwithPostgres, "test"))
