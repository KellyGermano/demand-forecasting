    ...
    def generate_all_of(self):
        ...
        for definition_item in self._definition["allOf"]:
            self.generate_func_code_block(
                definition_item, self._variable, self._variable_name, clear_variables=True
            )
    ...
    def generate_max_items(self):
        ...
        with self.l("if {variable}_is_list:"):
            ...
            with self.l("if {variable}_len > {maxItems}:"):
                self.exc(
                    "{name} must contain less than or equal to {maxItems} items", rule="maxItems"
                )
    ...
    def generate_min_properties(self):
        ...
        with self.l("if {variable}_is_dict:"):
            ...
            with self.l("if {variable}_len < {minProperties}:"):
                self.exc(
                    "{name} must contain at least {minProperties} properties", rule="minProperties"
                )
        ...
        with self.l("if {variable}_is_dict:"):
            ...
            if not self._definition.get("additionalProperties", True):
                not_possible = [
                    ...
                    if prop not in self._definition.get("properties", {})
                    and not any(
                        re.search(regex, prop)
                        for regex in self._definition.get("patternProperties", {})
                    )
                ]
                if not_possible:
                    raise JsonSchemaDefinitionException(
                        "{}: items {} are required but not allowed".format(
                            self._variable, not_possible
                        )
                    )
        ...
        with self.l("if {variable}_is_dict:"):
            ...
            for key, prop_definition in self._definition["properties"].items():
                ...
                if not isinstance(prop_definition, (dict, bool)):
                    raise JsonSchemaDefinitionException(
                        "{}[{}] must be object".format(self._variable, key_name)
                    )
                ...
                if (
                    ...
                    self.l(
                        'else: {variable}["{}"] = {}', self.e(key), repr(prop_definition["default"])
                    )
