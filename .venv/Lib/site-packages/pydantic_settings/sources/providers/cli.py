...
class _CliArg(BaseModel):
        ...
        **values: Any,
        ...
        if self.subcommand_dest:
            for sub_model in self.sub_models:
                ...
                parser_map[self.field_info][subcommand_alias] = parser_map[self.subcommand_dest][
                    subcommand_alias
                ]
    ...
    def subcommand_dest(self) -> Optional[str]:
        return (
            f"{self.arg_prefix}:subcommand" if _CliSubCommand in self.field_info.metadata else None
        )
...
class CliSettingsSource(EnvSettingsSource, Generic[T]):
    ...
    def _connect_root_parser(
        ...
        if parse_args_method is None:
            parse_args_method = (
                _parse_known_args if self.cli_ignore_unknown_args else ArgumentParser.parse_args
            )
        ...
        self._parser_map: defaultdict[str | FieldInfo, dict[Optional[int] | str, _CliArg]] = (
            defaultdict(dict)
        )
    ...
    def _add_parser_args(
        ...
        for field_name, field_info in self._sort_arg_fields(model):
            ...
            if arg.subcommand_dest:
                ...
                if arg.is_parser_submodel and not getattr(
                    field_info.annotation, "__pydantic_root_model__", False
                ):
    ...
    def _serialized_args(self, model: PydanticModel, _is_submodel: bool = False) -> list[str]:
        ...
        for field_name, field_info in _get_model_fields(
            ...
            if _CliPositionalArg in field_info.metadata:
                for value in model_default if isinstance(model_default, list) else [model_default]:
                    value = (
                        json.dumps(value) if isinstance(value, (dict, list, set)) else str(value)
                    )
        ...
        serialized_args += [
            json.dumps(value) if not isinstance(value, str) else value for value in optional_args
        ]
        serialized_args += [
            json.dumps(value) if not isinstance(value, str) else value for value in positional_args
        ]
