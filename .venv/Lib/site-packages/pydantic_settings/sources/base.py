...
class InitSettingsSource(PydanticBaseSettingsSource):
    ...
    def __init__(
        ...
        for field_name, field_info in settings_cls.model_fields.items():
            ...
            if init_kwarg_name:
                preferred_alias = alias_names[0]
                preferred_set_alias = next(
                    alias for alias in alias_names if alias in init_kwarg_name
                )
...
class PydanticBaseEnvSettingsSource(PydanticBaseSettingsSource):
    def __init__(
        ...
        self.env_parse_enums = (
            env_parse_enums if env_parse_enums is not None else self.config.get("env_parse_enums")
        )
    ...
    def _extract_field_info(self, field: FieldInfo, field_name: str) -> list[tuple[str, str, bool]]:
        ...
        if not v_alias or self.config.get("populate_by_name", False):
            ...
            if typing_objects.is_typealiastype(annotation) or typing_objects.is_typealiastype(
            ...
            if is_union_origin(get_origin(annotation)) and _union_is_complex(
                ...
                field_info.append(
                    (field_name, self._apply_case_sensitive(self.env_prefix + field_name), True)
                )
            else:
                field_info.append(
                    (field_name, self._apply_case_sensitive(self.env_prefix + field_name), False)
                )
