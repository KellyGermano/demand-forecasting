...
class BaseSettings(BaseModel):
    ...
    def _settings_build_values(
        ...
        env_file = (
            _env_file if _env_file != ENV_FILE_SENTINEL else self.model_config.get("env_file")
        )
        env_file_encoding = (
            _env_file_encoding
        ...
        cli_prog_name = (
            _cli_prog_name if _cli_prog_name is not None else self.model_config.get("cli_prog_name")
        )
        cli_parse_args = (
            _cli_parse_args
        ...
        cli_shortcuts = (
            _cli_shortcuts if _cli_shortcuts is not None else self.model_config.get("cli_shortcuts")
        )

        secrets_dir = (
            _secrets_dir if _secrets_dir is not None else self.model_config.get("secrets_dir")
        )
        ...
        **model_init_data: Any,
        ...
        if not issubclass(model_cls, BaseSettings):
            ...
            for field_name, field_info in base_settings_cls.model_fields.items():
                model_init_data[_field_name_for_signature(field_name, field_info)] = getattr(
                    model, field_name
                )
