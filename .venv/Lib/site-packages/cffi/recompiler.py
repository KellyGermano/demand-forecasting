...
class StructUnionExpr:
    def __init__(
        self, name, type_index, flags, size, alignment, comment, first_field_index, c_fields
    ):
        self.name = name
...
class Recompiler:
    ...
    def write_py_source_to_f(self, f):
        ...
        if num_includes > 0:
            prnt(
                "    _includes = (%s,)," % (", ".join(["_ffi%d" % i for i in range(num_includes)]),)
            )
        #
        # the footer
        prnt(")")
    ...
    def _generate_cpy_function_ctx(self, tp, name):
        ...
        self._lsts["global"].append(
            GlobalExpr(
                name, "_cffi_f_%s" % name, CffiOp(meth_kind, type_index), size="_cffi_d_%s" % name
            )
        )
    ...
    def _struct_ctx(self, tp, cname, approxname, named_ptr=None):
        ...
        if reason_for_not_expanding is None:
            ...
            for fldname, fldtype, fbitsize, fqual in enumfields:
                ...
                c_fields.append(
                    FieldExpr(fldname, offset, size, fbitsize, CffiOp(op, self._typesdict[fldtype]))
                )
        ...
        self._lsts["struct_union"].append(
            StructUnionExpr(
                tp.name, type_index, flags, size, align, comment, first_field_index, c_fields
            )
        )
    ...
    def _enum_ctx(self, tp, cname):
        ...
        for enumerator, enumvalue in zip(tp.enumerators, tp.enumvalues):
            self._lsts["global"].append(
                GlobalExpr(
                    enumerator, "_cffi_const_%s" % enumerator, type_op, check_value=enumvalue
                )
            )
        #
    ...
    **kwds,
    ...
    if preamble is not None:
        if call_c_compiler and _is_file_like(c_file):
            raise TypeError(
                "Writing to file-like objects is not supported " "with call_c_compiler=True"
            )
