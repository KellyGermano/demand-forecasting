...
class CParser(PLYParser):
    ...
    def _add_typedef_name(self, name, coord):
        ...
        if not self._scope_stack[-1].get(name, True):
            self._parse_error(
                "Typedef %r previously declared as non-typedef " "in this scope" % name, coord
            )
    ...
    def _build_declarations(self, spec, decls, typedef_namespace=False):
        ...
        # like an abstract declarator.  Give it a name if this is the case.
            ...
            decls[0]["decl"], (c_ast.Enum, c_ast.Struct, c_ast.Union, c_ast.IdentifierType)
        ):
        ...
        for decl in decls:
            ...
            if isinstance(
                declaration.type, (c_ast.Enum, c_ast.Struct, c_ast.Union, c_ast.IdentifierType)
            ):
        ...
        if p[2] is None:
            ...
            #
                ...
                decls = self._build_declarations(
                    spec=spec, decls=[dict(decl=None, init=None)], typedef_namespace=True
                )
    ...
    def p_direct_xxx_declarator_1(self, p):
        ...
        p[0] = c_ast.TypeDecl(
            declname=p[1], type=None, quals=None, align=None, coord=self._token_coord(p, 1)
        )
    ...
    def p_direct_xxx_declarator_3(self, p):
        ...
        # Per C99 6.7.5.3 p7
        arr = c_ast.ArrayDecl(
            type=None, dim=p[4] if len(p) > 5 else p[3], dim_quals=quals, coord=p[1].coord
        )
    ...
    def p_direct_abstract_declarator_4(self, p):
        ...
        arr = c_ast.ArrayDecl(
            type=None, dim=c_ast.ID(p[3], self._token_coord(p, 3)), dim_quals=[], coord=p[1].coord
        )
    ...
    def p_direct_abstract_declarator_7(self, p):
        ...
        p[0] = c_ast.FuncDecl(
            args=p[2], type=c_ast.TypeDecl(None, None, None, None), coord=self._token_coord(p, 1)
        )
    ...
    def p_direct_abstract_declarator_8(self, p):
        ...
        quals = [qual for sublist in listed_quals for qual in sublist if qual is not None]
        p[0] = c_ast.ArrayDecl(
            type=c_ast.TypeDecl(None, None, None, None),
            dim=p[4],
            dim_quals=quals,
    ...
    def p_labeled_statement_4(self, p):
        ...
        p[0] = c_ast.Label(
            p[1], c_ast.EmptyStatement(self._token_coord(p, 1)), self._token_coord(p, 1)
        )

    def p_labeled_statement_5(self, p):
        """labeled_statement : CASE constant_expression COLON"""
        p[0] = c_ast.Case(
            p[2], [c_ast.EmptyStatement(self._token_coord(p, 2))], self._token_coord(p, 1)
        )

    def p_labeled_statement_6(self, p):
        """labeled_statement : DEFAULT COLON"""
        p[0] = c_ast.Default(
            [c_ast.EmptyStatement(self._token_coord(p, 1))], self._token_coord(p, 1)
        )
    ...
    def p_iteration_statement_4(self, p):
        ...
        p[0] = c_ast.For(
            c_ast.DeclList(p[3], self._token_coord(p, 1)), p[4], p[6], p[8], self._token_coord(p, 1)
        )
