...
class Connection(object):
    ...
    def __init__(self, sock):
        ...
        try:
            ...
            with _lock:
                ...
                # not an error.
                if any(conn.pid == self.pid and not conn.process_replaced for conn in _connections):
    ...
    def authenticate(self):
        if access_token is None and adapter.access_token is None:
            return
        auth = self.channel.request("pydevdAuthorize", {"debugServerAccessToken": access_token})
        if auth["clientAccessToken"] != adapter.access_token:
            self.channel.close()
            raise RuntimeError('Mismatched "clientAccessToken"; server not authorized.')

    def request(self, request):
        raise request.isnt_valid("Requests from the debug server to the client are not allowed.")
    ...
    def disconnect(self):
        if self.connection.process_replaced:
            ...
            if not session.client or not session.client.is_connected:
                wait_for_connection(session, lambda conn: conn.pid == self.pid, timeout=60)
            else:
                self.wait_for(
                    lambda: (
                        not session.client
                        or not session.client.is_connected
                        or any(conn.pid == self.pid for conn in session.client.known_subprocesses)
                    ),
                    timeout=60,
                )
        with _lock:
