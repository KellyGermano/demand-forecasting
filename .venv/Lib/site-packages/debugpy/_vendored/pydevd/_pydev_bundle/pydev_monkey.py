    ...
    try:
        ...
        if code_i != -1:
            ...
            if port is not None:
                ...
                new_args.append(
                    _get_python_c_args(host, port, code, unquoted_args, SetupHolder.setup)
                )
...
class _NewThreadStartupWithTrace:
    ...
    def __call__(self):
        ...
        if py_db is not None:
            ...
            t = getattr(
                self.original_func, "__self__", getattr(self.original_func, "im_self", None)
            )
    ...
    for attr in replace_attrs:
        if attr in start_joinable_attrs:
            ...
            if getattr(thread_module, "_original_start_new_thread", None) is None:
                _original_start_new_thread = thread_module._original_start_new_thread = getattr(
                    thread_module, attr
                )
            else:
    ...
    class ClassWithPydevStartJoinableThread:
        def pydev_start_joinable_thread(self, function, *args, **kwargs):
            ...
            return _original_start_joinable_thread(
                _UseNewThreadStartup(function, args, kwargs), handle=handle, daemon=daemon
            )
