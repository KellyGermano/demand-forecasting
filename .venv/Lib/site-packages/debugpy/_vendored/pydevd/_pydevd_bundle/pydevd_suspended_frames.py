...
class _AbstractVariable(object):
    ...
    def get_var_data(
        self, fmt: Optional[dict] = None, context: Optional[str] = None, **safe_repr_custom_attrs
    ):
        """
        :param dict fmt:
...
class _FrameVariable(_AbstractVariable):
    ...
    def get_children_variables(self, fmt=None, scope=None):
        ...
        lst, group_entries = self._group_entries(
            [(x[0], x[1], None) for x in list(dct.items()) if x[0] != "_pydev_stop_at_break"],
        ...
        for key, val, _ in group_entries:
            ...
            variable = _ObjectVariable(
                self.py_db, key, val, self._register_variable, False, key, frame=self.frame
            )
...
class _FramesTracker(object):
    ...
    def track(self, thread_id, frames_list, frame_custom_thread_id=None):
        ...
        with self._lock:
            ...
            for frame in frames_list:
                ...
                self._suspended_frames_manager._variable_reference_to_frames_tracker[frame_id] = (
                    self
                )
    ...
    def create_thread_suspend_command(
        self, thread_id, stop_reason, message, trace_suspend_type, thread, additional_info
    ):
        with self._lock:
