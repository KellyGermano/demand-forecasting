...
def _run_with_interrupt_thread(original_func, py_db, curr_thread, frame, expression, is_exec):
    ...
    if interrupt_thread_timeout > 0:
        ...
        pydev_log.info(
            "Doing evaluate with interrupt threads timeout: %s.", interrupt_thread_timeout
        )
    ...
    def new_func(py_db, frame, expression, is_exec):
        ...
        def on_warn_evaluation_timeout():
            py_db.writer.add_command(
                py_db.cmd_factory.make_evaluation_timeout_msg(py_db, expression, curr_thread)
            )
...
def change_attr_expression(
    frame, attr, expression, dbg, value=SENTINEL_VALUE, /, scope: Optional[ScopeRequest] = None
):
