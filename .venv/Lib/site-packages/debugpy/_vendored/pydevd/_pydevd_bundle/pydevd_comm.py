    ...
    try:
        ...
        if not modules_to_reload:
            if filename and module_name:
                ...
                _send_io_message(
                    dbg, "code reload: Unable to find module to reload for path: %s\n" % (filename,)
                )
            elif module_name:
                _send_io_message(
                    dbg, "code reload: Unable to find module to reload: %s\n" % (module_name,)
                )
            ...
            for module, module_name in modules_to_reload.values():
                _send_io_message(
                    dbg, 'code reload: Start reloading module: "' + module_name + '" ... \n'
                )
                ...
                if pydevd_reload.xreload(module):
                    ...
                    _send_io_message(
                        dbg, "code reload: reload finished without applying any change\n"
                    )
...
def internal_get_variable_json(py_db, request):
    ...
    try:
        ...
        try:
            ...
            variables = [
                {"name": "<error>", "value": err, "type": "<error>", "variablesReference": 0}
            ]
        except:
...
class InternalGetArray(InternalThreadCommand):
    def __init__(
        self, seq, roffset, coffset, rows, cols, format, thread_id, frame_id, scope, attrs
    ):
        self.sequence = seq
        self.thread_id = thread_id
        self.frame_id = frame_id
    ...
    def do_it(self, dbg):
        try:
            ...
            xml = pydevd_vars.table_like_struct_to_xml(
                var, self.name, self.roffset, self.coffset, self.rows, self.cols, self.format
            )
...
def internal_change_variable_json(py_db, request):
    ...
    child_var = variable.change_variable(
        arguments.name, arguments.value, py_db, fmt=fmt, scope=scope
    )
...
def internal_evaluate_expression(
    dbg, seq, thread_id, frame_id, expression, is_exec, trim_if_too_big, attr_to_set_result
):
    """gets the value of a variable"""
    try:
        frame = dbg.find_frame(thread_id, frame_id)
...
def internal_set_expression_json(py_db, request, thread_id):
    ...
    if frame_tracker is None:
        ...
        _set_expression_response(
            py_db, request, error_message="Thread id: %s is not current thread id." % (thread_id,)
        )
        return
...
def build_exception_info_response(
            ...
            try:
                ...
                while frames_list is not None and len(frames_list):
                    ...
                    for (
                        ...
                        frames.append(
                            (filename_in_utf8, lineno, method_name, line_text, line_col_info)
                        )
                    ...
                    if IS_PY311_OR_GREATER:
                        ...
                        for (
                            ...
                            if line_col_info is not None:
                                ...
                                frame_summary = traceback.FrameSummary(
                                    filename_in_utf8, lineno, method_name, line=line_text
                                )
                                stack_summary.append(frame_summary)
                        ...
                        stack_str = "".join(
                            traceback.format_list((x[:-1] for x in frames[-max_frames:]))
                        )
    ...
    if current_paused_frame_name:
        name += "       (note: full exception trace is shown but execution is paused at: %s)" % (
            current_paused_frame_name,
        )
