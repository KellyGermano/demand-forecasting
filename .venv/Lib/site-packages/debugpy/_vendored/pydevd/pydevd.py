...
pydev_log.debug(
    "Using GEVENT_SHOW_PAUSED_GREENLETS: %s", pydevd_constants.GEVENT_SHOW_PAUSED_GREENLETS
)
...
class PyDB(object):
        ...
        try:
            if os.path.exists(filename):
                ...
                if try_except_infos:
                    ...
                    try_except_infos = [
                        x for x in try_except_infos if min_line <= x.try_line <= max_line
                    ]
    ...
    def _internal_get_file_type(self, abs_real_path_and_basename):
        ...
        if basename.startswith(IGNORE_BASENAMES_STARTING_WITH) or abs_real_path_and_basename[
            0
        ].startswith(IGNORE_BASENAMES_STARTING_WITH):
    ...
    def get_file_type(
        ...
        try:
            ...
            if abs_real_path_and_basename[0] == "<string>":
                f = frame.f_back
                while f is not None:
                    if self.get_file_type(f) != self.PYDEV_FILE and pydevd_file_utils.basename(
            ...
            if self._exclude_filters_enabled:
                ...
                if exclude_by_filter is not None:
                    if exclude_by_filter:
                        ...
                        if DEBUG:
                            pydev_log.debug_once(
                                "File not traced (excluded by filters): %s", original_filename
                            )
    ...
    def exclude_exception_by_filter(self, exception_breakpoint, trace):
        ...
        if (ignore_libraries and not self.in_project_scope(trace.tb_frame)) or (
            ...
            and self._exclude_by_filter(
                trace.tb_frame, pydevd_file_utils.absolute_path(trace.tb_frame.f_code.co_filename)
            )
        ):
            return True
    ...
    def do_wait_suspend(self, thread, frame, event, arg, exception_type=None):  # @UnusedVariable
        ...
        if DebugInfoHolder.DEBUG_TRACE_LEVEL > 2:
            ...
            for f in frames_list:
                pydev_log.debug(
                    "  Stack: %s, %s, %s", f.f_code.co_filename, f.f_code.co_name, f.f_lineno
                )
        ...
        with self.suspended_frames_manager.track_frames(self) as frames_tracker:
            ...
            with self._threads_suspended_single_notification.notify_thread_suspended(
                ...
                keep_suspended = self._do_wait_suspend(
                    thread, frame, event, arg, trace_suspend_type, from_this_thread, frames_tracker
                )
        ...
        if DebugInfoHolder.DEBUG_TRACE_LEVEL > 2:
            pydev_log.debug(
                "Leaving PyDB.do_wait_suspend: %s (%s) %s", thread, thread_id, id(thread)
            )
    ...
    def _do_wait_suspend(
        ...
        if info.pydev_step_cmd in (CMD_STEP_INTO, CMD_STEP_INTO_MY_CODE):
            ...
            info.pydev_step_cmd == CMD_RUN_TO_LINE or info.pydev_step_cmd == CMD_SET_NEXT_STATEMENT
        ):
            info.pydev_step_stop = None
            self.set_trace_for_frame_and_parents(thread.ident, frame)
            stop = False
            ...
            if force_check_project_scope or self.is_files_filter_enabled:
                while back_frame is not None:
                    if self.apply_files_filter(
                        back_frame, back_frame.f_code.co_filename, force_check_project_scope
                    ):
                        frame = back_frame
    ...
    def _exec(self, is_module, entry_point_fn, module_name, file, globals, locals):
        ...
        if not is_module:
            ...
            if entry_point_fn:
                mod = __import__(
                    module_name, level=0, fromlist=[entry_point_fn], globals=globals, locals=locals
                )
    ...
    def wait_for_commands(self, globals):
        ...
        frame = pydevd_frame_utils.Frame(
            None,
            -1,
            pydevd_frame_utils.FCode("Console", os.path.abspath(os.path.dirname(__file__))),
            globals,
            globals,
        )
        thread_id = get_current_thread_id(thread)
...
def config(protocol="", debug_mode="", preimport=""):
    pydev_log.debug(
        "Config: protocol: %s, debug_mode: %s, preimport: %s", protocol, debug_mode, preimport
    )
