...
def cmd_step_into(py_db, frame, event, info, thread, stop_info, stop):
    ...
    if _is_django_suspended(thread):
        ...
        if stop:
            info.pydev_django_resolve_frame = (
                True  # we remember that we've go into python code from django rendering frame
            )
    ...
    if py_db.django_exception_break and exception is not None:
        if (
            ...
            if exception.__name__ == "TemplateSyntaxError":
                ...
                if lineno is not None:
                    original_filename = _get_original_filename_from_origin_in_parent_frame_locals(
                        frame, "get_template"
                    )
