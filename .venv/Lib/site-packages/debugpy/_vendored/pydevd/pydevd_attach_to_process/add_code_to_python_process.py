...
def _win_write_to_shared_named_memory(python_code, pid):
    ...
    assert len(python_code) < BUFSIZE - 1, "Python code must have at most %s bytes (found: %s)" % (
        BUFSIZE - 1,
        len(python_code),
    )
    ...
    filemap = CreateFileMapping(
        INVALID_HANDLE_VALUE, 0, PAGE_READWRITE, 0, BUFSIZE, "__pydevd_pid_code_to_run__%s" % (pid,)
    )

    if filemap == INVALID_HANDLE_VALUE or filemap is None:
        raise Exception(
            "Failed to create named file mapping (ctypes: CreateFileMapping): %s" % (filemap,)
        )
    try:
