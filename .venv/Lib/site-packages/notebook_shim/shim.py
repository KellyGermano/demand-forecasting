...
def EXTAPP_AND_NBAPP_AND_SVAPP_SHIM_MSG(trait_name, extapp_name):
    return (
        ...
        "NotebookApp and ServerApp for your purpose.".format(
            trait_name=trait_name, extapp_name=extapp_name
        )
    )


def EXTAPP_AND_SVAPP_SHIM_MSG(trait_name, extapp_name):
    return (
        "'{trait_name}' is found in both {extapp_name} "
        "and ServerApp. This is a recent change. "
        "This config will only be set in {extapp_name}. "
        "Please check if you should also config these traits in "
        "ServerApp for your purpose.".format(trait_name=trait_name, extapp_name=extapp_name)
    )
...
def EXTAPP_AND_NBAPP_SHIM_MSG(trait_name, extapp_name):
    return (
        ...
        "This config will only be set in {extapp_name}. "
        "Please check if you should also config these traits in "
        "NotebookApp for your purpose.".format(trait_name=trait_name, extapp_name=extapp_name)
    )
...
def EXTAPP_TO_SVAPP_SHIM_MSG(trait_name, extapp_name):
    return (
        ...
        "our next release.".format(trait_name=trait_name, extapp_name=extapp_name)
    )
...
class NotebookConfigShimMixin:
    ...
    def shim_config_from_notebook_to_jupyter_server(self, config):
        ...
        extapp_traits = self.__class__.class_trait_names() + ExtensionApp.class_trait_names()
        svapp_traits = ServerApp.class_trait_names()
        nbapp_traits = NotebookAppTraits.class_trait_names() + ExtensionApp.class_trait_names()
        ...
        for trait_name, trait_value in extapp_config.items():
            ...
            if trait_name in IGNORED_TRAITS:
                ...
                warning_msg = EXTAPP_AND_NBAPP_AND_SVAPP_SHIM_MSG(trait_name, extapp_name)
                extapp_config_shim.update({trait_name: trait_value})
            elif in_extapp and in_svapp:
                warning_msg = EXTAPP_AND_SVAPP_SHIM_MSG(trait_name, extapp_name)
                extapp_config_shim.update({trait_name: trait_value})
            elif in_extapp and in_nbapp:
                warning_msg = EXTAPP_AND_NBAPP_SHIM_MSG(trait_name, extapp_name)
                extapp_config_shim.update({trait_name: trait_value})
            elif in_extapp:
                extapp_config_shim.update({trait_name: trait_value})
            elif in_svapp and in_nbapp:
                warning_msg = NOT_EXTAPP_NBAPP_AND_SVAPP_SHIM_MSG(trait_name, extapp_name)
                svapp_config_shim.update({trait_name: trait_value})
            elif in_svapp:
                warning_msg = EXTAPP_TO_SVAPP_SHIM_MSG(trait_name, extapp_name)
                svapp_config_shim.update({trait_name: trait_value})
            elif in_nbapp:
                warning_msg = EXTAPP_TO_NBAPP_SHIM_MSG(trait_name, extapp_name)
                nbapp_config_shim.update({trait_name: trait_value})
            else:
                raise TraitError("Trait, {}, not found.".format(trait_name))
        ...
        if extapp_config_shim:
            new_config.update(Config({self.__class__.__name__: extapp_config_shim}))
