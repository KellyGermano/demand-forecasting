...
class ForestToParseTree(ForestTransformer):
    ...
    def transform_packed_node(self, node, data):
        ...
        transformed = (
            children if node.parent.is_intermediate else self._call_rule_func(node, children)
        )
        if self._use_cache:
    ...
    def visit_packed_node_out(self, node):
        ...
        for child in [node.left, node.right]:
            if child is not None:
                ...
                child_graph_node_id = str(
                    randint(100000000000000000000000000000, 123456789012345678901234567890)
                )
                ...
                self.graph.add_edge(
                    self.pydot.Edge(graph_node, child_graph_node, style=child_edge_style)
                )
