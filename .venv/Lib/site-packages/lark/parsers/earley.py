...
class Parser:
    ...
    def parse(self, lexer, start):
        ...
        solutions = dedup_list(
            n.node
            for n in columns[-1]
            if n.is_complete and n.node is not None and n.s == start_symbol and n.start == 0
        )
        if not solutions:
        ...
        if len(solutions) > 1:
            raise RuntimeError(
                "Earley should not generate multiple start symbol items! Please report this bug."
            )
        (solution,) = solutions
