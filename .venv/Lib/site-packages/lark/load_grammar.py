...
RULES = {
    ...
    "_expansions": ["alias", "_expansions _OR alias", "_expansions _NL_OR alias"],
    ...
    "term": ["TERMINAL _COLON expansions _NL", "TERMINAL _DOT NUMBER _COLON expansions _NL"],
            ...
            if isinstance(child, Tree) and child.data == "expansions":
                ...
                tree.children = [
                    self.visit(
                        ST(
                            "expansion",
                            [option if i == j else other for j, other in enumerate(tree.children)],
                        )
                    )
...
class PrepareAnonTerminals(Transformer_InPlace):
    ...
    def pattern(self, p):
        ...
        if p in self.term_reverse and p.flags != self.term_reverse[p].pattern.flags:
            raise GrammarError("Conflicting flags for the same terminal: %s" % p)
...
def _literal_to_pattern(literal):
    ...
    if literal.type == "REGEXP" and "\n" in v and "x" not in flags:
        raise GrammarError(
            "You can only use newlines in regular expressions " "with the `x` (verbose) flag"
        )
    ...
    def compile(self, start, terminals_to_keep) -> Tuple[List[TerminalDef], List[Rule], List[str]]:
        ...
        while i < len(
            ...
            if len(params) != 0:  # Dont transform templates
                continue
            rule_options = (
                RuleOptions(keep_all_tokens=True) if options and options.keep_all_tokens else None
            )
        ...
        if terminals_to_keep != "*":
            used_terms = {
                t.name for r in compiled_rules for t in r.expansion if isinstance(t, Terminal)
            }
        ...
        for name, token_tree in term_dict.items():
            ...
            for exp in token_tree.find_data("value"):
                ...
                if isinstance(item, NonTerminal):
                    raise GrammarError(
                        "Rules aren't allowed inside terminals (%s in %s)" % (item, name)
                    )
...
def _get_parser():
    try:
        ...
        rules = [
            Rule(NonTerminal(r), [symbol_from_strcase(s) for s in x.split()], i, None, o)
            for r, xs, o in rules
...
def _make_rule_tuple(modifiers_tree, name, params, priority_tree, expansions):
    ...
    return (
        ...
        expansions,
        RuleOptions(
            keep_all_tokens, expand1, priority=priority, template_source=(name if params else None)
        ),
    )
...
class GrammarBuilder:
    ...
    def _extend(self, name, is_term, exp, params=(), options=None):
        if name not in self._definitions:
            self._grammar_error(
                is_term, "Can't extend {type} {name} as it wasn't defined before", name
            )
        ...
        if tuple(params) != d.params:
            self._grammar_error(
                is_term, "Cannot extend {type} with different parameters: {name}", name
            )
    ...
    def _unpack_import(self, stmt, grammar_name):
        ...
        if path_node.data == "import_lib":  # Import from library
            ...
            if (
                ...
                base_file = (
                    grammar_name  # Import relative to grammar file path if external grammar file
                )
            if base_file:
    ...
    def _unpack_definition(self, tree, mangle):
        ...
        if tree.data == "rule":
            ...
            opts = (
                int(tree.children[1]) if len(tree.children) == 3 else TOKEN_DEFAULT_PRIORITY
            )  # priority
            exp = tree.children[-1]
            is_term = True
    ...
    def do_import(
        ...
        to_try = (
            self.import_paths + ([base_path] if base_path is not None else []) + [stdlib_loader]
        )
        ...
        for name, d in self._definitions.items():
            ...
            for i, p in enumerate(params):
                if p in self._definitions:
                    raise GrammarError(
                        "Template Parameter conflicts with rule %s (in template %s)" % (p, name)
                    )
                if p in params[:i]:
                    raise GrammarError(
                        "Duplicate Template Parameter %s (in template %s)" % (p, name)
                    )
            ...
            for temp in exp.find_data("template_usage"):
                ...
                if sym not in params:
                    if sym not in self._definitions:
                        self._grammar_error(
                            d.is_term,
                            "Template '%s' used but not defined (in {type} {name})" % sym,
                            name,
                        )
                    if len(args) != len(self._definitions[sym].params):
                        ...
                        self._grammar_error(
                            ...
                            "(expected %s, got %s) (in {type2} {name2})" % (expected, actual),
                            sym,
                            name,
                        )
