...
class LanguageServerManager(LanguageServerManagerAPI):
    ...
    _ready = Bool(help="""Whether the manager has been initialized""", default_value=False)
    ...
    def subscribe(self, handler):
        ...
        if session is None:
            self.log.error(
                "[{}] no session: handler subscription failed".format(handler.language_server)
            )
            return
    ...
    async def on_client_message(self, message, handler):
        await self.wait_for_listeners(MessageScope.CLIENT, message, handler.language_server)
        session = self.sessions.get(handler.language_server)

        if session is None:
            self.log.error(
                "[{}] no session: client message dropped".format(handler.language_server)
            )
            return
    ...
    async def on_server_message(self, message, session):
        language_servers = [ls_key for ls_key, sess in self.sessions.items() if sess == session]

        for language_servers in language_servers:
            await self.wait_for_listeners(MessageScope.SERVER, message, language_servers)
    ...
    def unsubscribe(self, handler):
        ...
        if session is None:
            self.log.error(
                "[{}] no session: handler unsubscription failed".format(handler.language_server)
            )
            return
    ...
    def _autodetect_language_servers(self, only_installed: bool):
        ...
        if skipped_servers:
            self.log.info(
                _("Skipped non-installed server(s): {}").format(", ".join(skipped_servers))
            )
