...
class DockingBar(window.Wnd):
    ...
    def CalcDynamicLayout(self, length, mode):
        ...
        if self.IsFloating():
            ...
            if hittest == win32con.HTTOPLEFT:
                ...
                top = (
                    min(pt[1], windowRect[3] - self.cCaptionSize - self.cMinHeight) - self.cyBorder
                )
    ...
    def OnWindowPosChanged(self, msg):
        ...
        if (
            ...
            and ((self._obj_.dwStyle & afxres.CBRS_BORDER_ANY) != afxres.CBRS_BORDER_ANY)
        ):
            return
...
def test():
    ...
    bar.SetBarStyle(
        bar.GetBarStyle() | afxres.CBRS_TOOLTIPS | afxres.CBRS_FLYBY | afxres.CBRS_SIZE_DYNAMIC
    )
