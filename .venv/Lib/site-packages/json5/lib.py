...
class JSON5Encoder:
    ...
    def _encode_quoted_str(self, obj: str, quote_style: QuoteStyle) -> str:
        ...
        if double_quotes_seen > single_quotes_seen and quote_style == QuoteStyle.PREFER_DOUBLE:
            return self._encode_quoted_str(obj, QuoteStyle.ALWAYS_SINGLE)
        if single_quotes_seen > double_quotes_seen and quote_style == QuoteStyle.PREFER_SINGLE:
    ...
    def _encode_array(self, obj: Any, seen: Set, level: int) -> str:
        ...
        return (
            "["
            + indent_str
            + item_sep.join(self.encode(el, seen, level, as_key=False) for el in obj)
            + end_str
            + "]"
        )
    ...
    def is_identifier(self, key: str) -> bool:
        ...
        the changes yet."""
        if not key or not self._is_id_start(key[0]) and key[0] not in ("$", "_"):
            return False
        for ch in key[1:]:
