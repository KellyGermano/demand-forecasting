...
def start_wsgi_server(
    ...
    if certfile and keyfile:
        context = _get_ssl_ctx(
            certfile, keyfile, protocol, client_cafile, client_capath, client_auth_required
        )
...
def generate_latest(
    registry: CollectorRegistry = REGISTRY, escaping: str = openmetrics.UNDERSCORES
) -> bytes:
    """
    ...
    def sample_line(samples):
        ...
        if escaping != openmetrics.ALLOWUTF8 or openmetrics._is_valid_legacy_metric_name(
            samples.name
        ):
            if labelstr:
    ...
    for metric in registry.collect():
        ...
        for suffix, lines in sorted(om_samples.items()):
            ...
            output.append(
                f"# TYPE {openmetrics.escape_metric_name(metric.name + suffix, escaping)} gauge\n"
            )
            output.extend(lines)
        ...
        if accepted.split(";")[0].strip() == "application/openmetrics-text":
            ...
            if version and parse_version(version) >= (1, 0, 0):
                return (
                    ...
                    f"application/openmetrics-text; version={version}; charset=utf-8; escaping="
                    + str(escaping),
                )
...
def write_to_textfile(
    ...
    escaping: str = openmetrics.ALLOWUTF8,
    ...
    if tmpdir is not None:
        ...
        tmppath = (
            f"{os.path.join(tmpdir, filename)}.{os.getpid()}.{threading.current_thread().ident}"
        )
    else:
