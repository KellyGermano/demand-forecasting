...
class MetricWrapperBase(Collector):
    ...
    def collect(self) -> Iterable[Metric]:
        ...
        for suffix, labels, value, timestamp, exemplar, native_histogram_value in self._samples():
            metric.add_sample(
                self._name + suffix, labels, value, timestamp, exemplar, native_histogram_value
            )
        return [metric]
    ...
    def remove(self, *labelvalues: Any) -> None:
        ...
        if len(labelvalues) != len(self._labelnames):
            raise ValueError(
                "Incorrect label count (expected %d, got %s)" % (len(self._labelnames), labelvalues)
            )
        labelvalues = tuple(str(l) for l in labelvalues)
        with self._lock:
    ...
    def clear(self) -> None:
        ...
        if "prometheus_multiproc_dir" in os.environ or "PROMETHEUS_MULTIPROC_DIR" in os.environ:
            warnings.warn(
                "Clearing labels has not been implemented in multi-process mode yet", UserWarning
            )
        with self._lock:
            self._metrics = {}
