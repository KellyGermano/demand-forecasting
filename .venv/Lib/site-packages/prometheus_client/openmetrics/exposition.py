...
def _is_valid_exemplar_metric(metric, sample):
    ...
    if metric.type in ("gaugehistogram") and sample.name.endswith("_bucket"):
        return True
    if (
        metric.type in ("histogram")
        and sample.name.endswith("_bucket")
        or sample.name == metric.name
    ):
        return True
    return False
...
def generate_latest(registry, escaping=UNDERSCORES, version="1.0.0"):
    ...
    for metric in registry.collect():
        try:
            ...
            output.append(
                "# HELP {} {}\n".format(
                    escape_metric_name(mname, escaping),
                    _escape(metric.documentation, ALLOWUTF8, _is_legacy_labelname_rune),
                )
            )
            ...
            for s in metric.samples:
                ...
                if s.labels:
                    ...
                    labelstr += ",".join(
                        [
                            '{}="{}"'.format(
                                escape_label_name(k, escaping),
                                _escape(v, ALLOWUTF8, _is_legacy_labelname_rune),
                            )
                            for k, v in items
                        ]
                    )
                ...
                if s.native_histogram:
                    ...
                    if s.native_histogram.neg_spans:
                        negative_spans = ",".join(
                            [f"{ns[0]}:{ns[1]}" for ns in s.native_histogram.neg_spans]
                        )
                    ...
                    if s.native_histogram.pos_spans:
                        positive_spans = ",".join(
                            [f"{ps[0]}:{ps[1]}" for ps in s.native_histogram.pos_spans]
                        )
                ...
                if (escaping != ALLOWUTF8) or _is_valid_legacy_metric_name(s.name):
                    ...
                    output.append("{} {}{}{}\n".format(labelstr, value, timestamp, exemplarstr))
...
def _is_legacy_labelname_rune(b: str, i: int) -> bool:
    ...
    return ("a" <= b <= "z") or ("A" <= b <= "Z") or (b == "_") or ("0" <= b <= "9" and i > 0)
