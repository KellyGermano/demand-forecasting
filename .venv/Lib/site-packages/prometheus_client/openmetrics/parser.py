...
def _parse_nh_sample(text, suffixes):
    ...
    if has_metric_labels:
        ...
        if name.endswith(suffixes):
            raise ValueError(
                "the sample name of a native histogram with labels should have no suffixes", name
            )
        if not name:
            # Name might be in the labels
            if "__name__" not in labels:
    ...
    for line in fd:
        ...
        if line == "# EOF":
            ...
            if name + "_bucket" == sample.name and (
                sample.labels.get("le", "NaN") == "NaN" or _isUncanonicalNumber(sample.labels["le"])
            ):
            ...
            if (
                sample.name[len(name) :] in ["_total", "_sum", "_count", "_bucket", "_gcount"]
                and sample.value < 0
            ):
