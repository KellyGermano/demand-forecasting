...
def _get_token_collection(version_info):
    try:
        ...
        _token_collection_cache[tuple(version_info)] = result = _create_token_collection(
            version_info
        )
        return result
    ...
    *,
    ...
    for line in lines:  # loop over lines in stream
        ...
        if contstr:  # continued string
            ...
            if endmatch:
                pos = endmatch.end(0)
                yield PythonToken(STRING, contstr + line[:pos], contstr_start, prefix)  # noqa: F821
                contstr = ""
                contline = ""
            else:
        ...
        while pos < max_:
            ...
            if not pseudomatch:  # scan for tokens
                ...
                new_line = False
                yield PythonToken(
                    ERRORTOKEN, line[pos], (lnum, pos), additional_prefix + match.group(0)
                )
                additional_prefix = ""
                pos += 1
                continue
            ...
            #  triple quote checking (above).
                ...
                initial in single_quoted or token[:2] in single_quoted or token[:3] in single_quoted
            ):
                if token[-1] in "\r\n":  # continued string
                ...
                if token in "([{":
                    ...
                    token.startswith(":")
                    and fstring_stack
                    and fstring_stack[-1].parentheses_count - fstring_stack[-1].format_spec_count
                    == 1
                ):
                    # `:` and `:=` both count
