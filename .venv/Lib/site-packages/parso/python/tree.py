...
_FLOW_CONTAINERS = set(
    ["if_stmt", "while_stmt", "for_stmt", "try_stmt", "with_stmt", "async_stmt", "suite"]
)
...
class Scope(PythonBaseNode, DocstringMixin):
    ...
    def __repr__(self):
        ...
        return "<%s: %s@%s-%s>" % (type(self).__name__, name, self.start_pos[0], self.end_pos[0])
...
class ImportName(Import):
    ...
    def is_nested(self):
        ...
        """
        return bool(
            [1 for path, alias in self._dotted_as_names() if alias is None and len(path) > 1]
        )
    ...
    def _aliases(self):
        ...
        """
        return dict(
            (alias, path[-1]) for path, alias in self._dotted_as_names() if alias is not None
        )
