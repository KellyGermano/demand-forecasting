            ...
            if node.type != IndentationTypes.BACKSLASH:
                if node.type != IndentationTypes.SUITE:
                    ...
                    self._indentation_tos = BackslashNode(
                        self._config, indentation, part, spacing, parent=self._indentation_tos
                    )
        elif self._on_newline:
            ...
            if not self._check_tabs_spaces(spacing):
                ...
                if self._new_statement:
                    ...
                    if (
                        ...
                        and indentation
                        == node.get_latest_suite_node().indentation + self._config.indentation
                    ):
            ...
            if prev in _OPENING_BRACKETS:
                ...
                if (
                    ...
                    if param.type == "param" and param.annotation:
                        ...
                        add_if_spaces(
                            spacing, 251, "Unexpected spaces around keyword / parameter equals"
                        )
                elif part in _BITWISE_OPERATOR or prev in _BITWISE_OPERATOR:
                    add_not_spaces(
                        spacing, 227, "Missing whitespace around bitwise or shift operator"
                    )
                elif part == "%" or prev == "%":
            ...
            for node in left, right:
                if node.type == "keyword" or node.type == "name":
                    if node.value == "None":
                        ...
                        message = (
                            "comparison to False/True should be " "'if cond is True:' or 'if cond:'"
                        )
