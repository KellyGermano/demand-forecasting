...
def test_nonzero(strings, na_object):
    ...
    assert_array_equal(
        strings_with_na[strings_with_na.nonzero()], strings_with_na[strings_with_na.astype(bool)]
    )
...
def test_assign_medium_strings():
    ...
    N = 9
    src = np.array(
        (["0" * 256] * 3 + ["0" * 255] + ["0" * 256] + ["0" * 255] + ["0" * 256] * 2 + ["0" * 255]),
        dtype="T",
    )
    dst = np.array(
        (["0" * 255] + ["0" * 256] * 2 + ["0" * 255] + ["0" * 256] + ["0" * 255] + [""] * 5),
        dtype="T",
    )
...
def test_unset_na_coercion():
    ...
    for op_dtype in [None, StringDType(), StringDType(coerce=False), StringDType(na_object=None)]:
        if op_dtype is None:
            op = "2"
        else:
    ...
    for op_dtype in [None, StringDType(), StringDType(coerce=True), StringDType(na_object=None)]:
        if op_dtype is None:
            op = inp
        else:
