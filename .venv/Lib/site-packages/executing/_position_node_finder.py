...
class PositionNodeFinder(object):
    ...
    def fix_result(self, node: EnhancedAST, instruction: dis.Instruction) -> EnhancedAST:
        if (
    ...
    def verify(self, node: EnhancedAST, instruction: dis.Instruction) -> None:
        ...
        def node_match(node_type: Union[Type, Tuple[Type, ...]], **kwargs: Any) -> bool:
            ...
            return isinstance(node, node_type) and all(
                isinstance(getattr(node, k), v) if isinstance(v, type) else getattr(node, k) == v
        ...
        if (
            (
                ...
                or (node_match(ast.Name, ctx=ast.Store) and isinstance(node.parent, ast.AugAssign))
            )
            and inst_match(
                (
                    "LOAD_NAME",
        ...
        if sys.version_info >= (3, 12):
            ...
            if node_match(ast.TypeVar) and (
                ...
                or inst_match("CALL_INTRINSIC_2", argrepr="INTRINSIC_TYPEVAR_WITH_BOUND")
                or inst_match("CALL_INTRINSIC_2", argrepr="INTRINSIC_TYPEVAR_WITH_CONSTRAINTS")
            ...
            if node_match((ast.FunctionDef, ast.AsyncFunctionDef)) and node.type_params:
                ...
                if sys.version_info >= (3, 14):
                    if inst_match(
                        "LOAD_FAST_BORROW_LOAD_FAST_BORROW", argval=(".defaults", ".kwdefaults")
                    ):
                        return
            ...
            if node_match(ast.JoinedStr) and (
                inst_match("LOAD_ATTR", argval="join") or inst_match(("LIST_APPEND", "CALL"))
            ):
                return
    ...
    def instruction_before(self, instruction: dis.Instruction) -> Optional[dis.Instruction]:
    ...
    def find_node(
        ...
        return only(
            ...
            if all(getattr(position, attr) == getattr(node, attr) for attr in match_positions)
        )
