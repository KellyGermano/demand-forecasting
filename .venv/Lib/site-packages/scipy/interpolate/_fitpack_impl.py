...
from numpy import atleast_1d, array, ones, zeros, sqrt, ravel, transpose, empty, iinfo, asarray
...
_iermess = {
    0: ["The spline has a residual sum of squares fp such that " "abs(fp-s)/s<=0.001", None],
...
def splprep(
    ...
    if ier <= 0 and not quiet:
        warnings.warn(
            RuntimeWarning(_iermess[ier][0] + f"\tk={k} n={len(t)} m={m} fp={fp} s={s}"),
            stacklevel=2,
        )
...
_curfit_cache = {"t": array([], float), "wrk": array([], float), "iwrk": array([], dfitpack_int)}


def splrep(
    x, y, w=None, xb=None, xe=None, k=3, task=0, s=None, t=None, full_output=0, per=0, quiet=1
):
...
def bisplrep(
    ...
    tx, ty, c, o = _fitpack._surfit(
        x, y, z, w, xb, xe, yb, ye, kx, ky, task, s, eps, tx, ty, nxest, nyest, wrk, lwrk1, lwrk2
    )
    _curfit_cache["tx"] = tx
    _curfit_cache["ty"] = ty
    ...
    if ierm <= 0 and not quiet:
        _mess = (
            _iermess2[ierm][0] + f"\tkx,ky={kx},{ky} nx,ny={len(tx)},{len(ty)} m={m} fp={fp} s={s}"
        )
        warnings.warn(RuntimeWarning(_mess), stacklevel=2)
...
def splder(tck, n=1):
    ...
    if n > k:
        raise ValueError(
            f"Order of derivative (n = {n!r}) must be <= " f"order of spline (k = {tck[2]!r})"
        )
    ...
    with np.errstate(invalid="raise", divide="raise"):
        try:
            ...
            raise ValueError(
                "The spline has internal repeated knots " f"and is not differentiable {n} times"
            ) from e
