...
class TestBSpline:
    ...
    def test_ctor(self):
        ...
        assert_raises((TypeError, ValueError), BSpline, **dict(t=[1, 1.0j], c=[1.0], k=0))
        ...
        assert_raises(
            TypeError, BSpline, **dict(t=[0.0, 0.0, 1.0, 2.0, 3.0, 4.0], c=[1.0, 1.0, 1.0], k=2.5)
        )
    ...
    def test_degree_1(self):
        ...
        xp_assert_close(
            c[0] * B_012(x) + c[1] * B_012(x - 1) + c[2] * B_012(x - 2), b(x), atol=1e-14
        )
    ...
    def test_bernstein(self):
        ...
        xp_assert_close(bp(xx, extrapolate=True), bspl(xx, extrapolate=True), atol=1e-14)
    ...
    def test_endpoints(self, num_parallel_threads):
        ...
        for extrap in (True, False):
            xp_assert_close(
                b([tm, tp], extrap), b([tm + 1e-10, tp - 1e-10], extrap), atol=1e-9, rtol=1e-7
            )
    ...
    def test_continuity(self, num_parallel_threads):
        ...
        b = _make_random_spline()
        t, _, k = b.tck
        xp_assert_close(b(t[k + 1 : -k - 1] - 1e-10), b(t[k + 1 : -k - 1] + 1e-10), atol=1e-9)
    ...
    def test_antiderivative_method(self):
        ...
        t, c, k = b.tck
        xx = np.linspace(t[k], t[-k - 1], 20)
        xp_assert_close(b.antiderivative().derivative()(xx), b(xx), atol=1e-14, rtol=1e-14)
    ...
    def test_integral(self):
        ...
        xp_assert_close(
            b.integrate(1, -1, extrapolate=False), np.asarray(_impl.splint(1, -1, b.tck))
        )
        ...
        xp_assert_close(b.integrate(1.5 + 12, 3 + 12), np.asarray(i(1) - i(0) + i(2) - i(1.5)))
        xp_assert_close(
            b.integrate(1.5, 3 + 12), np.asarray(i(1) - i(0) + i(2) - i(1.5) + 6 * period_int)
        )
    ...
    def test_design_matrix_same_as_BSpline_call(self, extrapolate, degree):
        ...
        xp_assert_close(bspline(x), BSpline.design_matrix(x, t, k, extrapolate).toarray())
...
class TestInsert:
    ...
    def test_insert(self, xval):
        ...
        xp_assert_close(
            spl_yy1.c, np.c_[spl.insert_knot(xval).c, spl_y1.insert_knot(xval).c], atol=1e-15
        )
    ...
    @pytest.mark.parametrize("xval, m", [(0.0, 2), (1.0, 3), (1.5, 5), (4, 2), (7.0, 2)])
...
class TestInterop:
    ...
    def test_splev(self):
        ...
        xp_assert_close(np.asarray([splev(x, b) for x in xnew]), b(xnew), atol=1e-15, rtol=1e-15)
    ...
    def test_splint(self):
        ...
        xp_assert_close(splint(0, 1, b), splint(0, 1, b.tck), atol=1e-14, check_0d=False)
    ...
    def test_insert(self):
        ...
        xp_assert_close(np.asarray(splev(xx, tck_n2)).transpose(2, 0, 1), bn2(xx), atol=1e-15)
...
class TestInterp:
    ...
    def test_quadratic_deriv(self):
        ...
        xp_assert_close(b(self.xx), self.yy, atol=1e-14, rtol=1e-14)
        xp_assert_close(b(self.xx[-1], 1), der[0][1], atol=1e-14, rtol=1e-14, check_0d=False)

        # derivative at left-hand edge
        b = make_interp_spline(self.xx, self.yy, k=2, bc_type=(der, None))
        xp_assert_close(b(self.xx), self.yy, atol=1e-14, rtol=1e-14)
        xp_assert_close(b(self.xx[0], 1), der[0][1], atol=1e-14, rtol=1e-14, check_0d=False)
    ...
    def test_quintic_derivs(self):
        ...
        xp_assert_close(
            np.asarray([b(x[0], 1), b(x[0], 2)]), np.asarray([val for (nu, val) in der_l])
        )
        xp_assert_close(
            np.asarray([b(x[-1], 1), b(x[-1], 2)]), np.asarray([val for (nu, val) in der_r])
        )
    ...
    def test_knots_not_data_sites(self):
        ...
        k = 2
        t = np.r_[
            (self.xx[0],) * (k + 1), (self.xx[1:] + self.xx[:-1]) / 2.0, (self.xx[-1],) * (k + 1)
        ]
    ...
    def test_complex(self):
        ...
        xp_assert_close(b(xx), yy, atol=1e-14, rtol=1e-14)
        xp_assert_close(b(xx[0], 1), der_l[0][1], atol=1e-14, rtol=1e-14, check_0d=False)
        xp_assert_close(b(xx[-1], 1), der_r[0][1], atol=1e-14, rtol=1e-14, check_0d=False)
        ...
        for k in range(3, 32, 2):
            ...
            for i, j in enumerate(range(offset, -offset - 1, -1)):
                if j < 0:
                    ...
                    d[i, j:] = np.diagonal(a, offset=j)
            b = rng.random(n)
            xp_assert_close(_woodbury_algorithm(d, ur, ll, b, k), np.linalg.solve(a, b), atol=1e-14)
...
class TestGivensQR:
    ...
    def test_vs_full(self):
        ...
        # signs may differ
        xp_assert_close(np.minimum(R.todense() + r, R.todense() - r), np.zeros_like(r), atol=1e-15)
        xp_assert_close(
            np.minimum(abs(qTy - y_[:, 0]), abs(qTy + y_[:, 0])), np.zeros_like(qTy), atol=2e-13
        )
...
def bspline2(xy, t, c, k):
    ...
    ny = len(ty) - k - 1
    assert ny >= k + 1
    res = sum(c[ix, iy] * B(x, k, ix, tx) * B(y, k, iy, ty) for ix in range(nx) for iy in range(ny))
...
class TestNdBSpline:
    ...
    def test_2D_separable_2(self):
        ...
        xp_assert_close(bspl2_4(xi), np.asarray(target)[:, None], check_shape=False, atol=5e-14)
        ...
        xp_assert_close(result, [[val_single, val_single], [val_single, val_single]], atol=1e-14)
        ...
        xp_assert_close(
            bspl2_22(xi), np.asarray(target)[:, None, None], check_shape=False, atol=5e-14
        )
    ...
    def test_2D_random(self):
        ...
        xp_assert_close(spl(xi), [bspline2(xy, (tx, ty), c, k) for xy in xi], atol=1e-14)
    ...
    def test_3D_derivative(self):
        ...
        xp_assert_close(
            bspl3(xi, nu=(1, 0, 0)), 3 * x**2 * (y**3 + 2 * y) * (z**3 + 3 * z + 1), atol=1e-14
        )

        xp_assert_close(
            bspl3(xi, nu=(2, 0, 0)), 6 * x * (y**3 + 2 * y) * (z**3 + 3 * z + 1), atol=1e-14
        )

        xp_assert_close(
            bspl3(xi, nu=(2, 1, 0)), 6 * x * (3 * y**2 + 2) * (z**3 + 3 * z + 1), atol=1e-14
        )

        xp_assert_close(bspl3(xi, nu=(2, 1, 3)), 6 * x * (3 * y**2 + 2) * (6), atol=1e-14)
...
class TestMakeND:
    ...
    def test_2D_separable_trailing_dims(self):
        ...
        xp_assert_close(result.reshape(6, 6, 2, 2), target.reshape((6, 6, 2, 2)), atol=1e-14)
    ...
    @pytest.mark.parametrize("k, meth", [(1, "linear"), (3, "cubic_legacy"), (5, "quintic_legacy")])
...
class TestGenerateKnots:
    ...
    def test_nest(self):
        ...
        xp_assert_close(knots[-1], [0.0, 0.0, 0.0, 0.0, 2.0, 4.0, 7.0, 7.0, 7.0, 7.0], atol=1e-15)
