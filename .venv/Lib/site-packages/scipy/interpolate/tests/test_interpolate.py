...
class TestInterp1D:
    ...
    def test_validation(self):
        ...
        # Bad fill values
        assert_raises(
            ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=(-1, -1, -1)
        )  # doesn't broadcast
        assert_raises(
            ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=[-1, -1, -1]
        )  # doesn't broadcast
        ...
        assert_raises(
            ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=np.array([])
        )  # doesn't broadcast
        assert_raises(
            ValueError, interp1d, self.x10, self.y10, kind="linear", fill_value=()
        )  # doesn't broadcast
        assert_raises(
            ValueError, interp1d, self.x2, self.y210, kind="linear", axis=0, fill_value=[-1, -1]
        )  # doesn't broadcast
        assert_raises(
            ValueError,
            interp1d,
    ...
    def _check_linear(self, kind):
        ...
        xp_assert_close(extrapolator([-1.0, 0, 9, 11]), np.asarray([-1.0, 0, 9, 11]), rtol=1e-14)
            ...
            for dty in dt_rc:
                ...
                for dtn in dt_r:
                    ...
                    for kind in spline_kinds:
                        ...
                        xp_assert_close(
                            f(xnew), y, atol=1e-7, check_dtype=False, err_msg=f"{dtx}, {dty} {dtn}"
                        )
    ...
    def test_previous(self):
        ...
        xp_assert_close(interpolator1D([-1, -2, 5, 8, 12, 25]), [np.nan, np.nan, 5, 8, 9, 9])
        ...
        xp_assert_close(interpolator2DAxis0([-2, 5, 12]), [[np.nan, np.nan], [10, 11], [18, 19]])
        ...
        interpolator1D = interp1d(
            [0, 1, 2], [0, 1, -1], kind="previous", fill_value="extrapolate", assume_sorted=True
        )
        xp_assert_close(interpolator1D([-2, -1, 0, 1, 2, 3, 5]), [np.nan, np.nan, 0, 1, -1, -1, -1])
        ...
        interpolator2DAxis0 = interp1d(
            self.x10, self.y102_edge_updated, kind="previous", axis=0, fill_value="extrapolate"
        )
        xp_assert_close(interpolator2DAxis0([-2, 5, 12]), [[np.nan, np.nan], [10, 11], [-30, -30]])
    ...
    def test_next(self):
        ...
        xp_assert_close(interpolator1D([-1, -2, 5, 8, 12, 25]), [0, 0, 5, 8, np.nan, np.nan])
        ...
        xp_assert_close(interpolator2DAxis0([-2, 5, 12]), [[0, 1], [10, 11], [np.nan, np.nan]])
        ...
        # Tests for gh-16813
        interpolator1D = interp1d(
            [0, 1, 2], [0, 1, -1], kind="next", fill_value="extrapolate", assume_sorted=True
        )
        xp_assert_close(interpolator1D([-2, -1, 0, 1, 2, 3, 5]), [0, 0, 0, 1, -1, np.nan, np.nan])
        ...
        interpolator2DAxis0 = interp1d(
            self.x10, self.y102_edge_updated, kind="next", axis=0, fill_value="extrapolate"
        )
        xp_assert_close(interpolator2DAxis0([-2, 5, 12]), [[30, 30], [10, 11], [np.nan, np.nan]])
    ...
    def _bounds_check(self, kind="linear"):
        ...
        extrap10 = interp1d(
            self.x10, self.y10, fill_value=self.fill_value, bounds_error=False, kind=kind
        )
    ...
    def _check_fill_value(self, kind):
        ...
        for y in (self.y235, self.y325, self.y225, self.y25):
            ...
            interp = interp1d(
                self.x5, y, kind=kind, axis=-1, fill_value=(-100, 100), bounds_error=False
            )
            assert_array_almost_equal(interp(10), np.asarray(100.0))
            assert_array_almost_equal(interp(-10), np.asarray(-100.0))
            if y.ndim == 3:
        ...
        interp = interp1d(
            self.x5, self.y235, kind=kind, axis=-1, fill_value=fill_value, bounds_error=False
        )
        ...
        for y in (self.y225, self.y325, self.y25):
            interp = interp1d(
                self.x5, y, kind=kind, axis=-1, fill_value=fill_value, bounds_error=False
            )
            result = [100, 200]
            if y.ndim == 3:
        ...
        for ii in range(2):
            ...
            interp = interp1d(
                self.x5, self.y225, kind=kind, axis=-1, fill_value=fill_value, bounds_error=False
            )
            assert_array_almost_equal(interp(10), [[100, 200], [1000, 2000]])
            assert_array_almost_equal(interp(-10), [[100, 200], [1000, 2000]])
            assert_array_almost_equal(
                interp([-10, 10]), [[[100, 100], [200, 200]], [[1000, 1000], [2000, 2000]]]
            )
    ...
    def _nd_check_interp(self, kind="linear"):
        ...
        interp10 = interp1d(self.x10, self.y10, kind=kind)
        assert_array_almost_equal(
            interp10(np.array([[3.0, 5.0], [2.0, 7.0]])), np.array([[3.0, 5.0], [2.0, 7.0]])
        )
        ...
        assert_array_almost_equal(
            interp210(x_new), np.array([[[3.0, 5.0], [2.0, 7.0]], [[13.0, 15.0], [12.0, 17.0]]])
        )
        assert_array_almost_equal(
            interp102(x_new), np.array([[[6.0, 7.0], [10.0, 11.0]], [[4.0, 5.0], [14.0, 15.0]]])
        )
...
class TestPPoly:
    ...
    def test_antiderivative_simple(self):
        ...
        iic = np.array(
            [[1 / 4, 1 / 3, 1 / 2, 0, 0], [0, 0, 1.6875 / 2, 0.328125, 0.037434895833333336]]
        ).T
        x = np.array([0, 0.25, 1])
    ...
    def test_antiderivative_vs_derivative(self):
        ...
        for dx in range(0, 10):
            ...
            for k in range(dx):
                ...
                xp_assert_close(pp2(pp2.x[1:]), pp2(endpoint), rtol=1e-7, err_msg=f"dx={dx} k={k}")
    ...
    def test_integrate_periodic(self):
        ...
        xp_assert_close(P.integrate(3.5 + 12, 5 + 12), np.asarray(I(2) - I(1) + I(4) - I(3.5)))
        xp_assert_close(
            P.integrate(3.5, 5 + 12), np.asarray(I(2) - I(1) + I(4) - I(3.5) + 4 * period_int)
        )
...
class TestBPoly:
    ...
    def test_simple3(self):
        ...
        xp_assert_close(bp(0.2), np.asarray(3 * 0.8 * 0.8 + 1 * 2 * 0.2 * 0.8 + 4 * 0.2 * 0.2))
    ...
    def test_simple4(self):
        ...
        bp = BPoly(c, x)
        xp_assert_close(
            bp(0.3), np.asarray(0.7**3 + 3 * 0.7**2 * 0.3 + 3 * 0.7 * 0.3**2 + 2 * 0.3**3)
        )
    ...
    def test_simple5(self):
        ...
        xp_assert_close(
            bp(0.3),
            np.asarray(
                0.7**4 + 4 * 0.7**3 * 0.3 + 8 * 6 * 0.7**2 * 0.3**2 + 2 * 4 * 0.7 * 0.3**3 + 0.3**4
            ),
        )
    ...
    def test_interval_length(self):
        ...
        xp_assert_close(
            bp(xval), np.asarray(3 * (1 - s) * (1 - s) + 1 * 2 * s * (1 - s) + 4 * s * s)
        )
...
class TestBPolyCalculus:
    def test_derivative(self):
        ...
        xp_assert_close(
            np.asarray([bp(0.4, nu) for nu in [1, 2, 3]]), np.asarray([-6 * (1 - 0.4), 6.0, 0.0])
        )
        xp_assert_close(np.asarray([bp(1.7, nu) for nu in [1, 2, 3]]), np.asarray([0.7, 1.0, 0]))
    ...
    def test_der_antider(self):
        ...
        xp_assert_close(bp.antiderivative().derivative()(xx), bp(xx), atol=1e-12, rtol=1e-12)
    ...
    def test_antider_ppoly(self):
        ...
        xp_assert_close(bp.antiderivative(2)(xx), pp.antiderivative(2)(xx), atol=1e-12, rtol=1e-12)
    ...
    def test_integrate(self):
        ...
        pp = PPoly.from_bernstein_basis(bp)
        xp_assert_close(
            bp.integrate(0, 1), pp.integrate(0, 1), atol=1e-12, rtol=1e-12, check_0d=False
        )
    ...
    def test_integrate_extrap(self):
        ...
        xp_assert_close(
            b1.integrate(0, 2, extrapolate=True), np.asarray(2.0), atol=1e-14, check_0d=False
        )
    ...
    def test_integrate_periodic(self):
        ...
        xp_assert_close(P.integrate(3.5, 5 + 12), I(2) - I(1) + I(4) - I(3.5) + 4 * period_int)
    ...
    def test_antider_neg(self):
        ...
        xp_assert_close(b.derivative(-1)(xx), b.antiderivative()(xx), atol=1e-12, rtol=1e-12)
        xp_assert_close(b.derivative(1)(xx), b.antiderivative(-1)(xx), atol=1e-12, rtol=1e-12)
...
class TestNdPPoly:
    ...
    def test_integrate_2d(self):
        ...
        for ranges in [
            ...
            ig2, err2 = nquad(
                lambda x, y: p((x, y)), ranges, opts=[dict(epsrel=1e-5, epsabs=1e-5)] * 2
            )
            xp_assert_close(ig, ig2, rtol=1e-5, atol=1e-5, check_0d=False, err_msg=repr(ranges))
            ...
            for k2 in range(c.shape[1]):
                for k3 in range(c.shape[2]):
                    for k4 in range(c.shape[3]):
                        val += (
                            c[mx - k1 - 1, my - k2 - 1, mz - k3 - 1, mu - k4 - 1, j1, j2, j3, j4]
                            * _dpow(s1, k1, nu[0])
                            * _dpow(s2, k2, nu[1])
                            * _dpow(s3, k3, nu[2])
                            * _dpow(s4, k4, nu[3])
