...
class TestRegularGridInterpolator:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                _get_sample_4d,
                np.asarray([[0.1, 0.1, 1.0, 0.9], [0.2, 0.1, 0.45, 0.8], [0.5, 0.5, 0.5, 0.5]]),
            ),
    ...
    def test_derivatives(self):
        ...
        sample = np.array([[0.1, 0.1, 1.0, 0.9], [0.2, 0.1, 0.45, 0.8], [0.5, 0.5, 0.5, 0.5]])
        interp = RegularGridInterpolator(points, values, method="slinear")
        ...
        xp_assert_close(interp(sample, nu=(1, 0, 0, 0)), np.asarray([1.0, 1, 1]), atol=1e-15)
        xp_assert_close(interp(sample, nu=(0, 1, 0, 0)), np.asarray([10.0, 10, 10]), atol=1e-15)

        # 2nd derivatives of a linear function are zero
        xp_assert_close(interp(sample, nu=(0, 1, 1, 0)), np.asarray([0.0, 0, 0]), atol=2e-12)
    ...
    def test_length_one_axis(self):
        ...
        xp_assert_close(
            interp(np.array([[1, 1], [1, 5], [1, 10]])), np.asarray([2.0, 6, 11]), atol=1e-14
        )

        # check off-grid interpolation is indeed linear
        xp_assert_close(interp(np.array([[1, 1.4], [1, 5.3], [1, 10]])), [2.4, 6.3, 11], atol=1e-14)
        ...
        xp_assert_close(interp([[1.5, 1.8], [-4, 5.1]]), np.asarray([3.0, 6]), atol=1e-15)
    ...
    def test_float32_values(self, dtype, xi_dtype):
        ...
        xp_assert_close(
            interp(pts), [134.10469388, 153.40069388], atol=1e-7, rtol=1e-7, check_dtype=False
        )
    ...
    def test_bad_solver(self):
        ...
        with assert_raises(TypeError):
            # unknown argument
            RegularGridInterpolator((x, y), data, method="slinear", solver=lambda x: x, woof="woof")

        with assert_raises(TypeError):
            # unknown argument
            RegularGridInterpolator((x, y), data, method="slinear", solver_args={"woof": 42})
...
class TestInterpN:
    ...
    def test_spline_2d_outofbounds(self):
        ...
        y = np.array([0.5, 2.0, 3.0, 4.0, 5.5])
        z = np.array(
            [[1, 2, 1, 2, 1], [1, 2, 1, 2, 1], [1, 2, 3, 2, 1], [1, 2, 2, 2, 1], [1, 2, 1, 2, 1]]
        )
        lut = RectBivariateSpline(x, y, z)
    ...
    def test_4d_linear_outofbounds(self):
        ...
        actual = interpn(
            points, values, sample, method="linear", bounds_error=False, fill_value=999.99
        )
    ...
    def test_4d_nearest_outofbounds(self):
        ...
        wanted = np.asarray([999.99])
        actual = interpn(
            points, values, sample, method="nearest", bounds_error=False, fill_value=999.99
        )
        assert_array_almost_equal(actual, wanted)
    ...
    def test_nonscalar_values_2(self, method):
        ...
        vs = [
            [
                interpn(points, values[..., i, j], sample, method=method, bounds_error=False)
                for i in range(values.shape[-2])
            ]
            for j in range(values.shape[-1])
        ]
