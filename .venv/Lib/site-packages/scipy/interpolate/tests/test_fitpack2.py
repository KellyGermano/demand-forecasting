...
class TestUnivariateSpline:
    ...
    def test_derivatives_2(self):
        ...
        xp_assert_close(
            ders,
            [45.0, 39.0, 22.0, 6.0],  # 3**3 + 2*(3)**2  # 3*(3)**2 + 4*(3)  # 6*(3) + 4  # 6*3**0
            atol=1e-15,
        )
    ...
    def test_nan(self):
        ...
        for z in [np.nan, np.inf, -np.inf]:
            ...
            assert_raises(
                ValueError, InterpolatedUnivariateSpline, **dict(x=x, y=y, w=w, check_finite=True)
            )
            assert_raises(
                ValueError, LSQUnivariateSpline, **dict(x=x, y=y, t=t, w=w, check_finite=True)
            )
    ...
    def test_array_like_input(self):
        ...
        spl2 = UnivariateSpline(
            x=x_values.tolist(), y=y_values.tolist(), w=w_values.tolist(), bbox=bbox.tolist()
        )
...
class TestSmoothBivariateSpline:
    ...
    def test_integral(self):
        ...
        assert_almost_equal(
            np.asarray(lut2.integral(tx[0], tx[-1], ty[0], ty[-1])), np.asarray(trpz), decimal=0
    ...
    def test_array_like_input(self):
        ...
        spl2 = SmoothBivariateSpline(
            x.tolist(), y.tolist(), z.tolist(), bbox=bbox.tolist(), w=w.tolist(), kx=1, ky=1
        )
...
class TestSmoothSphereBivariateSpline:
    ...
    def test_invalid_input(self):
        ...
        with assert_raises(ValueError) as exc_info:
            invalid_phi = array(
                [-0.1 * pi, pi, 1.5 * pi, 0.5 * pi, pi, 1.5 * pi, 0.5 * pi, pi, 1.5 * pi]
            )
...
class TestRectBivariateSpline:
    ...
    @pytest.mark.parametrize("s_tols", [(0, 1e-12, 1e-7), (1, 7e-3, 1e-4), (3, 2e-2, 1e-4)])
    ...
    @pytest.mark.slow()
    @pytest.mark.skipif(
        sys.maxsize <= 2**32, reason="Segfaults on 32-bit system " "due to large input data"
    )
...
class TestRectSphereBivariateSpline:
    ...
    def test_derivatives_grid(self):
        ...
        xp_assert_close(lut(x, y, dtheta=1), _numdiff_2d(lut, x, y, dx=1), rtol=1e-4, atol=1e-4)
        xp_assert_close(lut(x, y, dphi=1), _numdiff_2d(lut, x, y, dy=1), rtol=1e-4, atol=1e-4)
        xp_assert_close(
        ...
        xp_assert_equal(
            lut(x, y, dtheta=1, grid=False), lut.partial_derivative(1, 0)(x, y, grid=False)
        )
        xp_assert_equal(
            lut(x, y, dphi=1, grid=False), lut.partial_derivative(0, 1)(x, y, grid=False)
        )
        xp_assert_equal(
            lut(x, y, dtheta=1, dphi=1, grid=False), lut.partial_derivative(1, 1)(x, y, grid=False)
        )
