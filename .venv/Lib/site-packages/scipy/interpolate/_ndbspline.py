    ...
    def __call__(self, xi, *, nu=None, extrapolate=None):
        ...
        if nu is None:
            ...
            if nu.ndim != 1 or nu.shape[0] != ndim:
                raise ValueError(
                    f"invalid number of derivative orders {nu = } for " f"ndim = {len(self.t)}."
                )
            if any(nu < 0):
        ...
        _strides_c1 = np.asarray([s // c1.dtype.itemsize for s in c1.strides], dtype=np.int64)
...
def _preprocess_inputs(k, t_tpl):
    ...
    if not isinstance(t_tpl, tuple):
        raise ValueError(f"Expect `t` to be a tuple of array-likes. " f"Got {t_tpl} instead.")
    ...
    for d in range(ndim):
        ...
        if td.ndim != 1:
            raise ValueError(f"Knot vector in dimension {d} must be" f" one-dimensional.")
        if n < kd + 1:
            raise ValueError(
                f"Need at least {2*kd + 2} knots for degree" f" {kd} in dimension {d}."
            )
        if (np.diff(td) < 0).any():
            raise ValueError(f"Knots in dimension {d} must be in a" f" non-decreasing order.")
        ...
        if not np.isfinite(td).all():
            raise ValueError(f"Knots in dimension {d} should not have" f" nans or infs.")
...
def make_ndbspl(points, values, k=3, *, solver=ssl.gcrotmk, **solver_args):
    ...
    t = tuple(_not_a_knot(np.asarray(points[d], dtype=float), k[d]) for d in range(ndim))
