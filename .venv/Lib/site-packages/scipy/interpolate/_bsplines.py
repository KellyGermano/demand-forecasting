    ...
    **Implementation details**
    ...
    def design_matrix(cls, x, t, k, extrapolate=False):
        ...
        if t.ndim != 1 or np.any(t[1:] < t[:-1]):
            raise ValueError(f"Expect t to be a 1-D sorted array_like, but " f"got t={t}.")
        ...
        return csr_array((data, indices, indptr), shape=(x.shape[0], t.shape[0] - k - 1))
    ...
    def integrate(self, a, b, extrapolate=None):
        ...
        if extrapolate == "periodic":
            ...
            if n_periods > 0:
                ...
                out = _dierckx.evaluate_spline(ta, ca.reshape(ca.shape[0], -1), ka, x, 0, False)
            ...
            # Evaluate the difference of antiderivatives.
            x = np.asarray([a, b], dtype=np.float64)
            out = _dierckx.evaluate_spline(ta, ca.reshape(ca.shape[0], -1), ka, x, 0, extrapolate)
            integral = out[1] - out[0]
    ...
    def from_power_basis(cls, pp, bc_type="not-a-knot"):
        ...
        if not isinstance(pp, CubicSpline):
            raise NotImplementedError(
                f"Only CubicSpline objects are accepted " f"for now. Got {type(pp)} instead."
            )
        x = pp.x
        coef = pp.c
        k = pp.c.shape[0] - 1
...
def make_interp_spline(x, y, k=3, t=None, bc_type=None, axis=0, check_finite=True):
    ...
    if bc_type == "periodic" and t is not None:
        raise NotImplementedError(
            "For periodic case t is constructed " "automatically and can not be passed " "manually"
        )
    ...
    if nright > 0:
        _handle_lhs_derivatives(t, k, x[-1], ab, kl, ku, deriv_r_ords, offset=nt - nright)
...
def make_lsq_spline(x, y, t, k=3, w=None, axis=0, check_finite=True, *, method="qr"):
    ...
    **NaN handling**: If the input arrays contain ``nan`` values, the result is
    ...
    if method == "norm-eq":
        ...
        m = rhs.shape[0]
        c = cho_solve_banded(
            (cho_decomp, lower), rhs.reshape(m, -1), overwrite_b=True, check_finite=check_finite
        ).reshape(rhs.shape)
    elif method == "qr":
...
def _compute_optimal_gcv_parameter(X, wE, y, w):
    ...
    if y.ndim == 1:
        gcv_est = minimize_scalar(_gcv, bounds=(0, n), method="Bounded", args=(X, XtWX, wE, XtE, y))
        if gcv_est.success:
            return gcv_est.x
...
def make_smoothing_spline(x, y, w=None, lam=None, *, axis=0):
    ...
    for j in range(2, n - 2):
        wE[:, j] = (
            (x[j + 2] - x[j - 2]) * _coeff_of_divided_diff(x[j - 2 : j + 3]) / w[j - 2 : j + 3]
        )
