    ...
    for i, p in enumerate(points):
        ...
        if not np.all(p[1:] > p[:-1]):
            if np.all(p[1:] < p[:-1]):
                ...
                raise ValueError(
                    f"The points in dimension {i} must be strictly ascending or " f"descending"
                )
...
def _check_dimensionality(points, values):
    if len(points) > values.ndim:
        raise ValueError(
            f"There are {len(points)} point arrays, but values has " f"{values.ndim} dimensions"
        )
    for i, p in enumerate(points):
        ...
        if not values.shape[i] == len(p):
            raise ValueError(
                f"There are {len(p)} points and {values.shape[i]} values in " f"dimension {i}"
            )
...
class RegularGridInterpolator:
    ...
    **Interpolate and extrapolate a 2D dataset**
    ...
    _SPLINE_METHODS_recursive = {"slinear_legacy", "cubic_legacy", "quintic_legacy", "pchip"}
    ...
    def _construct_spline(self, method, solver=None, **solver_args):
        if solver is None:
            solver = ssl.gcrotmk
        spl = make_ndbspl(
            self.grid, self.values, self._SPLINE_DEGREE_MAP[method], solver=solver, **solver_args
        )
        return spl
    ...
    def __call__(self, xi, method=None, *, nu=None):
        ...
        if method == "linear":
            ...
            if (
                ...
                result = evaluate_linear_2d(self.values, indices, norm_distances, self.grid, out)
            else:
    ...
    def _prepare_xi(self, xi):
        ...
        if self.bounds_error:
            for i, p in enumerate(xi.T):
                if not np.logical_and(np.all(self.grid[i][0] <= p), np.all(p <= self.grid[i][-1])):
    ...
    def _evaluate_nearest(self, indices, norm_distances):
        idx_res = [np.where(yi <= 0.5, i, i + 1) for i, yi in zip(indices, norm_distances)]
...
def interpn(points, values, xi, method="linear", bounds_error=True, fill_value=np.nan):
    ...
    if ndim > 2 and method == "splinef2d":
        raise ValueError("The method splinef2d can only be used for " "2-dimensional input data")
    ...
    if len(points) != ndim and method == "splinef2d":
        raise ValueError(
            "The method splinef2d can only be used for " "scalar data with one point per coordinate"
        )
