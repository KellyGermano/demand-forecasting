...
class _PPolyBase:
    ...
    def extend(self, c, x):
        ...
        if c.shape[2:] != self.c.shape[2:] or c.ndim != self.c.ndim:
            raise ValueError(f"Shapes of c {c.shape} and self.c {self.c.shape} are incompatible")
        ...
        c2 = np.zeros((k2, self.c.shape[1] + c.shape[1]) + self.c.shape[2:], dtype=dtype)
...
class PPoly(_PPolyBase):
    ...
    def from_bernstein_basis(cls, bp, extrapolate=None):
        ...
        if not isinstance(bp, BPoly):
            raise TypeError(
                f".from_bernstein_basis only accepts BPoly instances. " f"Got {type(bp)} instead."
            )
...
class BPoly(_PPolyBase):
    ...
    def from_power_basis(cls, pp, extrapolate=None):
        ...
        if not isinstance(pp, PPoly):
            raise TypeError(
                f".from_power_basis only accepts PPoly instances. " f"Got {type(pp)} instead."
            )
