    ...
    def __init__(
        ...
        if np.isscalar(smoothing):
            ...
            if smoothing.shape != (ny,):
                raise ValueError("Expected `smoothing` to be a scalar or have shape " f"({ny},).")
        ...
        if chunksize <= nx:
            ...
            for i in range(0, nx, chunksize):
                vec = _build_evaluation_coefficients(
                    x[i : i + chunksize, :], y, self.kernel, self.epsilon, self.powers, shift, scale
                )
    ...
    def __call__(self, x):
        ...
        if self.neighbors is None:
            out = self._chunk_evaluator(
                x, self.y, self._shift, self._scale, self._coeffs, memory_budget=memory_budget
            )
        else:
