...
def xp_result_type(*args, force_floating=False, xp):
    ...
    """
    args = [(_asarray(arg, subok=True, xp=xp) if np.iterable(arg) else arg) for arg in args]
...
def xp_promote(*args, broadcast=False, force_floating=False, xp):
    ...
    args = [
        (_asarray(arg, dtype=dtype, subok=True, xp=xp) if arg is not None else arg) for arg in args
    ]
...
def _make_sphinx_capabilities(
    ...
    capabilities = {
        ...
        "dask.array": _XPSphinxCapability(
            cpu=True, gpu=None, warnings=["computes graph"] if allow_dask_compute else []
        ),
    }
...
def xp_capabilities(
    *,
    ...
    """
    capabilities_table = xp_capabilities_table if capabilities_table is None else capabilities_table
...
def _make_xp_pytest_marks(*funcs, capabilities_table=None):
    ...
    for func in funcs:
        ...
        if capabilities["cpu_only"]:
            marks.append(
                pytest.mark.skip_xp_backends(cpu_only=True, exceptions=exceptions, reason=reason)
            )
        if capabilities["np_only"]:
            marks.append(
                pytest.mark.skip_xp_backends(np_only=True, exceptions=exceptions, reason=reason)
            )
