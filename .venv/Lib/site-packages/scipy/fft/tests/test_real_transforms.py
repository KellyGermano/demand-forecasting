...
@skip_xp_backends(np_only=True, reason="`overwrite_x` only supported for NumPy backend.")
@pytest.mark.parametrize("forward, backward", [(dct, idct), (dst, idst)])
@pytest.mark.parametrize("type", [1, 2, 3, 4])
...
def test_identity_1d_overwrite(forward, backward, type, dtype, axis, norm, overwrite_x, xp):
    # Test the identity f^-1(f(x)) == x
...
def test_identity_nd_overwrite(forward, backward, type, shape, axes, dtype, norm, overwrite_x, xp):
    # Test the identity f^-1(f(x)) == x
