...
class TestFFT:
    ...
    def test_fft2(self, xp):
        ...
        xp_assert_close(
            fft.fft2(x, norm="ortho"), expect / xp.sqrt(xp.asarray(30 * 20, dtype=xp.float64))
        )
    ...
    def test_ifft2(self, xp):
        ...
        xp_assert_close(
            fft.ifft2(x, norm="ortho"), expect * xp.sqrt(xp.asarray(30 * 20, dtype=xp.float64))
        )
    ...
    def test_fftn(self, xp):
        ...
        xp_assert_close(
            fft.fftn(x, norm="ortho"), expect / xp.sqrt(xp.asarray(30 * 20 * 10, dtype=xp.float64))
        )
    ...
    def test_ifftn(self, xp):
        ...
        xp_assert_close(fft.ifftn(x, norm="forward"), expect * (30 * 20 * 10), rtol=1e-7)
        ...
        for n in [xp_size(x), 2 * xp_size(x)]:
            for norm in [None, "backward", "ortho", "forward"]:
                xp_assert_close(
                    ...
                    fft.fft(xp.asarray(x, dtype=xp.complex128), n=n, norm=norm)[: (n // 2 + 1)],
                )
            xp_assert_close(
    ...
    def test_rfft2(self, xp):
        ...
        xp_assert_close(
            fft.rfft2(x, norm="ortho"), expect / xp.sqrt(xp.asarray(30 * 20, dtype=xp.float64))
        )
    ...
    def test_rfftn(self, xp):
        ...
        xp_assert_close(
            fft.rfftn(x, norm="ortho"), expect / xp.sqrt(xp.asarray(30 * 20 * 10, dtype=xp.float64))
        )
    ...
    def test_hfft(self, xp):
        ...
        xp_assert_close(
            fft.hfft(x_herm, norm="ortho"), expect / xp.sqrt(xp.asarray(30, dtype=xp.float64))
        )
    ...
    def test_ihfft2(self, xp):
        ...
        xp_assert_close(
            fft.ihfft2(x, norm="ortho"), expect * xp.sqrt(xp.asarray(30 * 20, dtype=xp.float64))
        )
    ...
    @pytest.mark.parametrize(
        "op",
        [fft.fft2, fft.ifft2, fft.rfft2, fft.irfft2, fft.hfft2, fft.ihfft2, fft.hfftn, fft.ihfftn],
    )
...
@skip_xp_backends(np_only=True)
@pytest.mark.parametrize(
    "dtype", [np.float32, np.float64, np.longdouble, np.complex64, np.complex128, np.clongdouble]
)
