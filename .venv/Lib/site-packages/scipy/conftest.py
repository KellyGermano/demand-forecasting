...
xp_known_backends = {"numpy", "array_api_strict", "torch", "cupy", "jax.numpy", "dask.array"}
...
if SCIPY_ARRAY_API:
    ...
    if isinstance(SCIPY_ARRAY_API, str) and SCIPY_ARRAY_API.lower() not in ("1", "true", "all"):
        ...
        if SCIPY_ARRAY_API_ != ["all"]:
            ...
            try:
                xp_available_backends = {
                    backend: xp_available_backends[backend] for backend in SCIPY_ARRAY_API_
                }
            except KeyError:
                msg = (
...
def xp(request, monkeypatch):
    ...
    if SCIPY_ARRAY_API:
        ...
        patch_lazy_xp_functions(xp=request.param, request=request, monkeypatch=monkeypatch)
...
def _backends_kwargs_from_request(request, skip_or_xfail):
    ...
    for marker in markers:
        ...
        if marker.kwargs.get("np_only", False):
            ...
            reason = marker.kwargs.get("reason") or ("eager checks not executed on lazy backends")
        ...
        if len(marker.args) == 1:
            ...
            if backend not in xp_known_backends:
                raise ValueError(
                    f"Unknown backend: {backend}; " f"must be one of {list(xp_known_backends)}"
                )
            reason = marker.kwargs.get("reason") or (
            ...
            raise ValueError(f"Please specify only one backend per marker: {marker.args}")
...
def skip_or_xfail_xp_backends(
    ...
    skip_xfail_reasons = _backends_kwargs_from_request(request, skip_or_xfail=skip_or_xfail)
    xp = request.param
...
if HAVE_SCPDT:
    ...
    def warnings_errors_and_rng(test=None):
        ...
        for name in divide_by_zero:
            known_warnings[name] = dict(category=RuntimeWarning, message="divide by zero")
        ...
        for name in integration_w:
            known_warnings[name] = dict(
                category=integrate.IntegrationWarning, message="The occurrence of roundoff"
            )
        ...
        dct = {
            "scipy.sparse.linalg.norm": dict(category=UserWarning, message="Exited at iteration"),
            # tutorials
            "linalg.rst": dict(
                message="the matrix subclass is not", category=PendingDeprecationWarning
            ),
            "stats.rst": dict(
                message="The maximum number of subdivisions", category=integrate.IntegrationWarning
            ),
        }
