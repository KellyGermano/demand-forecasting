...
def kaiserord(ripple, width):
    ...
    if A < 8:
        ...
        raise ValueError(
            "Requested maximum ripple attenuation " f"{A:f} is too small for the Kaiser formula."
        )
    beta = kaiser_beta(A)
...
def firwin(numtaps, cutoff, *, width=None, window="hamming", pass_zero=True, scale=True, fs=None):
    ...
    if pass_zero in ("bandstop", "lowpass"):
        if pass_zero == "lowpass":
            if xp_size(cutoff) != 1:
                raise ValueError(
                    "cutoff must have one element if " f'pass_zero=="lowpass", got {cutoff.shape}'
                )
        ...
        if pass_zero == "highpass":
            if xp_size(cutoff) != 1:
                raise ValueError(
                    "cutoff must have one element if " f'pass_zero=="highpass", got {cutoff.shape}'
                )
        elif xp_size(cutoff) <= 1:
            raise ValueError(
                "cutoff must have at least two elements if "
...
def firwin2(numtaps, freq, gain, *, nfreqs=None, window="hamming", antisymmetric=False, fs=None):
    """
    FIR filter design using the window method.
...
def remez(
    numtaps, bands, desired, *, weight=None, type="bandpass", maxiter=25, grid_density=16, fs=None
):
    """
    ...
    result = _sigtools._remez(numtaps, bands, desired, weight, tnum, fs, maxiter, grid_density)
...
def firls(numtaps, bands, desired, *, weight=None, fs=None):
    ...
    if len(weight) != len(desired):
        raise ValueError(
            "weight must be the same size as the number of " f"band pairs ({len(bands)})."
        )
    if (weight < 0).any():
...
def minimum_phase(
    ...
    *,
    ...
    if not xp.any(xp.flip(h[-n_half:]) - h[:n_half] <= 1e-8 + 1e-6 * abs(h[:n_half])):
        warnings.warn(
            "h does not appear to by symmetric, conversion may fail", RuntimeWarning, stacklevel=2
        )
...
def firwin_2d(hsize, window, *, fc=None, fs=2, circular=False, pass_zero=True, scale=True):
    ...
    if circular:
        if fc is None:
            raise ValueError("Cutoff frequency `fc` must be " "provided when `circular` is True")
