...
def lombscargle(
    ...
    *,
    ...
    if not (x.ndim == 1 and x.size > 0 and x.shape == y.shape == weights.shape):
        raise ValueError("Parameters x, y, weights must be 1-D arrays of " "equal non-zero length!")
    ...
    if normalize not in ["power", "normalize", "amplitude"]:
        raise ValueError(
            "Normalize must be: False (or 'power'), True (or 'normalize'), " "or 'amplitude'."
        )
    ...
    if Pxy.shape[-1] > 1:
        if average == "median":
            ...
            if np.iscomplexobj(Pxy):
                Pxy = np.median(np.real(Pxy), axis=-1) + np.median(np.imag(Pxy), axis=-1) * 1j
            else:
...
def coherence(
    x, y, fs=1.0, window="hann", nperseg=None, noverlap=None, nfft=None, detrend="constant", axis=-1
):
    r"""
...
def _spectral_helper(
    ...
    if return_onesided:
        if np.iscomplexobj(x):
            sides = "twosided"
            warnings.warn("Input data is complex, switching to return_onesided=False", stacklevel=3)
        else:
            sides = "onesided"
            if not same_data:
                if np.iscomplexobj(y):
                    sides = "twosided"
                    warnings.warn(
                        "Input data is complex, switching to " "return_onesided=False", stacklevel=3
                    )
    else:
        sides = "twosided"
