...
def _calc_dual_canonical_window(win: np.ndarray, hop: int) -> np.ndarray:
    ...
    if hop > len(win):
        raise ValueError(
            f"{hop=} is larger than window length of {len(win)}" + " => STFT not invertible!"
        )
...
def closest_STFT_dual_window(
    win: np.ndarray, hop: int, desired_dual: np.ndarray | None = None, *, scaled: bool = True
    ...
    if not (1 <= hop <= len(win) and isinstance(hop, int | np.integer)):
        raise ValueError(f"Parameter {hop=} is not an integer between 1 and " + f"{len(win)=}!")
    ...
    def from_win_equals_dual(
        ...
        *,
        ...
        if not (desired_win.ndim == 1 and desired_win.size > 0):
            raise ValueError(f"Parameter desired_win is not 1d, but " f"{desired_win.shape=}!")
        ...
        if not (1 <= hop <= len(desired_win) and isinstance(hop, int | np.integer)):
            raise ValueError(
                f"Parameter {hop=} is not an integer between 1 and " + f"{len(desired_win)=}!"
            )
        if scale_to not in ["magnitude", "psd", "unitary", None]:
            raise ValueError(
                f"Parameter {scale_to=} not in " + "['magnitude', 'psd', 'unitary', None]!"
            )
        ...
        for m in range(hop):
            ...
            if not (a > relative_resolution):
                raise ValueError(
                    "Parameter desired_win does not have valid STFT dual " f"window for {hop=}!"
                )
            win[m::hop] /= a
    ...
    def istft(
        ...
        *,
        ...
        if not (S.shape[t_axis] >= (q_num := self.p_num(n_min))):
            raise ValueError(
                f"{S.shape[t_axis]=} needs to have at least " + f"{q_num} slices ({S.shape=})!"
            )
        ...
        if not (self.k_min <= k0 < k1 <= k_max):
            raise ValueError(f"({self.k_min=}) <= ({k0=}) < ({k1=}) <= " + f"({k_max=}) is false!")
        ...
        q0 = k0 // self.hop + self.p_min if k0 >= 0 else k0 // self.hop  # p_min always <= 0
