...
def _minimize_slsqp(
    ...
    **unknown_options,
    ...
    if bounds is None or len(bounds) == 0:
        ...
        bnds = np.array([(_arr_to_scalar(lo), _arr_to_scalar(up)) for (lo, up) in bounds], float)
        if bnds.shape[0] != n:
            raise IndexError(
        ...
        if bnderr.any():
            raise ValueError(
                "SLSQP Error: lb > ub in bounds " f"{', '.join(str(b) for b in bnderr)}."
            )
    ...
    while True:
        ...
        if state_dict["iter"] > iter_prev:
            ...
            if iprint >= 2:
                print(f"{state_dict['iter']:5d} {sf.nfev:5d} " f"{fx:16.6E} {lanorm(g):16.6E}")
