...
class DifferentialEvolutionSolver:
    ...
    def init_population_array(self, init):
        ...
        if np.size(popn, 0) < 5 or popn.shape[1] != self.parameter_count or len(popn.shape) != 2:
            raise ValueError(
                "The population supplied needs to have shape" " (S, len(x)), where S > 4."
            )
    ...
    def solve(self):
        ...
        if np.all(np.isinf(self.population_energies)):
            self.feasible, self.constraint_violation = self._calculate_population_feasibilities(
                self.population
            )

            # only work out population energies for feasible solutions
            self.population_energies[self.feasible] = self._calculate_population_energies(
        ...
        for nit in range(1, self.maxiter + 1):
            ...
            if self.disp:
                print(f"differential_evolution step {nit}: f(x)=" f" {self.population_energies[0]}")
        ...
        if self._wrapped_constraints:
            ...
            if DE_result.maxcv > 0:
                ...
                DE_result.message = (
                    "The solution does not satisfy the " f"constraints, MAXCV = {DE_result.maxcv}"
                )
    ...
    def _accept_trial(
        self, energy_trial, feasible_trial, cv_trial, energy_orig, feasible_orig, cv_orig
    ):
        """
