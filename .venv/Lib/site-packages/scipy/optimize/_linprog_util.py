...
def _clean_inputs(lp):
    ...
    try:
        ...
        raise TypeError(
            "Invalid input for linprog: c must be a 1-D array of numerical " "coefficients"
        ) from e
    else:
        ...
        if not np.isfinite(c).all():
            raise ValueError(
                "Invalid input for linprog: c must not contain values " "inf, nan, or None"
            )
    ...
    try:
        ...
        if (
            ...
            raise ValueError(
                "Invalid input for linprog: A_ub must not contain values " "inf, nan, or None"
            )
    ...
    try:
        ...
        if not np.isfinite(b_ub).all():
            raise ValueError(
                "Invalid input for linprog: b_ub must not contain values " "inf, nan, or None"
            )
    ...
    try:
        ...
        if (
            ...
            raise ValueError(
                "Invalid input for linprog: A_eq must not contain values " "inf, nan, or None"
            )
    ...
    try:
        ...
        if not np.isfinite(b_eq).all():
            raise ValueError(
                "Invalid input for linprog: b_eq must not contain values " "inf, nan, or None"
            )
    ...
    if x0 is not None:
        try:
            ...
            raise TypeError(
                "Invalid input for linprog: x0 must be a 1-D array of " "numerical coefficients"
            ) from e
        if x0.ndim == 0:
            x0 = x0.reshape(-1)
        ...
        if not x0.size == c.size:
            raise ValueError(
                "Invalid input for linprog: x0 and c should contain the " "same number of elements"
            )
        if not np.isfinite(x0).all():
            raise ValueError(
                "Invalid input for linprog: x0 must not contain values " "inf, nan, or None"
            )
