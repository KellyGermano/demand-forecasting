...
class TestFSolve:
    ...
    def test_pressure_network_with_gradient(self):
        ...
        final_flows = optimize.fsolve(
            pressure_network, initial_guess, args=(Qtot, k), fprime=pressure_network_jacobian
        )
        assert_array_almost_equal(final_flows, np.ones(4))
...
class TestRootHybr:
    ...
    def test_pressure_network_with_gradient_combined(self):
        ...
        initial_guess = array([2.0, 0.0, 2.0, 0.0])
        final_flows = optimize.root(
            pressure_network_fun_and_grad, initial_guess, args=(Qtot, k), method="hybr", jac=True
        ).x
        assert_array_almost_equal(final_flows, np.ones(4))
...
class TestLeastSq:
    ...
    def test_float32(self):
        ...
        x = np.array(
            [1.475, 1.429, 1.409, 1.419, 1.455, 1.519, 1.472, 1.368, 1.286, 1.231], dtype=np.float32
        )
        y = np.array(
            [0.0168, 0.0193, 0.0211, 0.0202, 0.0171, 0.0151, 0.0185, 0.0258, 0.034, 0.0396],
            dtype=np.float32,
        )
...
class TestCurveFit:
    ...
    def test_pcov(self):
        ...
        for method in ["lm", "trf", "dogbox"]:
            ...
            popt, pcov = curve_fit(
                f, xdata, ydata, p0=[2, 0], sigma=sigma, absolute_sigma=True, method=method
            )
            perr = np.sqrt(np.diag(pcov))
            assert_allclose(perr, [0.30714756, 0.85045308], rtol=1e-3)

            popt, pcov = curve_fit(
                f, xdata, ydata, p0=[2, 0], sigma=3 * sigma, absolute_sigma=True, method=method
            )
            perr = np.sqrt(np.diag(pcov))
    ...
    def test_NaN_handling(self):
        ...
        assert_raises(
            ValueError, curve_fit, lambda x, a, b: a * x + b, xdata, ydata, **{"check_finite": True}
        )
    ...
    def test_curvefit_omitnan(self, sigma_dim):
        ...
        x[i_x] = np.nan
        y[i_y] = np.nan
        res_opt, res_cov = curve_fit(exponential, x, y, p0=p0, sigma=sigma, nan_policy="omit")
        ...
        for jac1, jac2 in [(jac, jac), (None, None)]:
            for absolute_sigma in [False, True]:
                popt1, pcov1 = curve_fit(
                    func, xdata, ydata, sigma=sigma, jac=jac1, absolute_sigma=absolute_sigma
                )
                popt2, pcov2 = curve_fit(
                    func, xdata, ydata, sigma=covar, jac=jac2, absolute_sigma=absolute_sigma
                )
    ...
    def test_curvefit_covariance(self):
        ...
        for jac1, jac2 in [(jac, jacp), (None, None)]:
            for absolute_sigma in [False, True]:
                ...
                popt2, pcov2 = curve_fit(
                    funcp, xdata, ydatap, sigma=covarp, jac=jac2, absolute_sigma=absolute_sigma
                )
