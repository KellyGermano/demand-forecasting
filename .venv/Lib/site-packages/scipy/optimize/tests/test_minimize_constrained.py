...
from numpy.testing import assert_array_almost_equal, assert_array_less, assert_, suppress_warnings
from scipy.optimize import NonlinearConstraint, LinearConstraint, Bounds, minimize, BFGS, SR1, rosen
...
class Elec:
    ...
    def hess(self, x):
        ...
        H = np.vstack(
            (np.hstack((Hxx, Hxy, Hxz)), np.hstack((Hxy, Hyy, Hyz)), np.hstack((Hxz, Hyz, Hzz)))
        )
...
class TestTrustRegionConstr:
    ...
    def test_list_of_problems(self, prob, grad, hess):
        ...
        sensitive = (
            isinstance(prob, BoundedRosenbrock) and grad == "3-point" and isinstance(hess, BFGS)
        )
        if sensitive:
...
def test_gh11649():
    ...
    nlcs = [NonlinearConstraint(nci, -10, np.inf), NonlinearConstraint(nce, 1, 1, jac=nce_jac)]
