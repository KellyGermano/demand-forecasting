...
from scipy.optimize._differentialevolution import DifferentialEvolutionSolver, _ConstraintWrapper
...
class TestDifferentialEvolutionSolver:
    ...
    def test_invalid_functional(self):
        ...
        with assert_raises(RuntimeError, match=r"func\(x, \*args\) must return a scalar value"):
    ...
    def test_rng_gives_repeatability(self):
        result = differential_evolution(self.quadratic, [(-100, 100)], polish=False, rng=1, tol=0.5)
    ...
    def test_population_initiation(self):
        ...
        assert_raises(
            ValueError, DifferentialEvolutionSolver, *(rosen, self.bounds), **{"init": "rubbish"}
        )
        ...
        solver = DifferentialEvolutionSolver(
            rosen, self.bounds, init=population, strategy="best2bin", atol=0.01, rng=1, popsize=5
        )
        ...
        assert_raises(
            ValueError, DifferentialEvolutionSolver, *(rosen, self.bounds), **{"init": population}
        )
    ...
    def test_deferred_updating(self):
        ...
        res = differential_evolution(rosen, bounds, updating="deferred", strategy="best1exp")
        assert res.success
    ...
    def test_accept_trial(self):
        ...
        assert not (fn(1.0, False, np.array([0.5, 0.5]), 1.0, False, np.array([1.0, 0.4])))
    ...
    def test_L1(self):
        ...
        res = differential_evolution(
            f, bounds, strategy="best1bin", rng=12345, constraints=(L,), popsize=5, tol=0.01
        )
        ...
        # using a lower popsize to speed the test up
        res = differential_evolution(
            f, bounds, strategy="best1bin", rng=1211134, constraints=(L,), popsize=2, tol=0.05
        )
    ...
    def test_L3(self):
        ...
        with suppress_warnings() as sup:
            sup.filter(UserWarning)
            res = differential_evolution(f, bounds, rng=1234, constraints=constraints, popsize=3)
    ...
    def test_L8(self):
        ...
        with suppress_warnings() as sup:
            ...
            # things up a lot
            res = differential_evolution(
                f, bounds, strategy="best1bin", rng=1234, constraints=constraints, maxiter=5000
            )
    ...
    def test_vectorized(self):
        ...
        with warns(UserWarning, match="differential_evolution: the 'workers"):
            differential_evolution(
                quadratic_vec, self.bounds, vectorized=True, workers=map, updating="deferred"
            )
    ...
    def test_vectorized_constraints(self):
        ...
        res1 = differential_evolution(
            rosen, bounds, updating="deferred", rng=1, constraints=[nlc1, nlc2], polish=False
        )
        res2 = differential_evolution(
            rosen_vec,
            bounds,
