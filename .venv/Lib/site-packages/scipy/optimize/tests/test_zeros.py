...
class TestScalarRootFinders:
    ...
    def run_tests(self, tests, method, name, known_fail=None, **kwargs):
        ...
        notclose = [
            ...
            if not isclose(a, c, rtol=rtol, atol=atol) and elt[-1]["ID"] not in known_fail
        ]
...
class TestBracketMethods(TestScalarRootFinders):
    ...
    def test_basic_root_scalar(self, method, function):
        ...
        r = root_scalar(
            function, method=method.__name__, bracket=[a, b], x0=a, xtol=self.xtol, rtol=self.rtol
        )
        assert r.converged
...
def test_gh_5555():
    ...
    for method in methods:
        ...
        assert_allclose(root, res, atol=xtol, rtol=rtol, err_msg=f"method {method.__name__}")
...
@pytest.mark.parametrize("method", [zeros.bisect, zeros.ridder, zeros.brentq, zeros.brenth])
