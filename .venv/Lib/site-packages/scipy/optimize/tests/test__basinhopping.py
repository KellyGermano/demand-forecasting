...
class TestBasinHopping:
    ...
    def test_1d_grad(self):
        ...
        i = 0
        res = basinhopping(
            func1d, self.x0[i], minimizer_kwargs=self.kwargs, niter=self.niter, disp=self.disp
        )
    ...
    def test_2d(self):
        ...
        i = 1
        res = basinhopping(
            func2d, self.x0[i], minimizer_kwargs=self.kwargs, niter=self.niter, disp=self.disp
        )
        assert_almost_equal(res.x, self.sol[i], self.tol)
        assert_(res.nfev > 0)
    ...
    def test_njev(self):
        ...
        res = basinhopping(
            func2d, self.x0[i], minimizer_kwargs=minimizer_kwargs, niter=self.niter, disp=self.disp
        )
        assert_(res.nfev > 0)
    ...
    def test_monotonic_basin_hopping(self):
        ...
        res = basinhopping(
            func1d, self.x0[i], minimizer_kwargs=self.kwargs, niter=self.niter, disp=self.disp, T=0
        )
        assert_almost_equal(res.x, self.sol[i], self.tol)
...
class Test_Metropolis:
    ...
    def test_gh7799(self):
        ...
        res = basinhopping(
            func, x0, 30, seed=np.random.RandomState(1234), minimizer_kwargs={"constraints": con}
        )
        assert res.success
