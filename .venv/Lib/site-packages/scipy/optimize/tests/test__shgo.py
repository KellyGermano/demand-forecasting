...
test4_1 = StructTest4(
    ...
    * 7,
    expected_x=[2.330499, 1.951372, -0.4775414, 4.365726, -0.6244870, 1.038131, 1.594227],
...
class StructTest5(StructTestFunction):
    def f(self, x):
        return -(x[1] + 47.0) * np.sin(np.sqrt(abs(x[0] / 2.0 + (x[1] + 47.0)))) - x[0] * np.sin(
...
def run_test(
    ...
    if test.expected_x is not None:
        np.testing.assert_allclose(res.x, test.expected_x, rtol=test_atol, atol=test_atol)
...
class TestShgoArguments:
    ...
    def test_4_1_known_f_min(self):
        ...
        options = {"f_min": test4_1.expected_fun, "f_tol": 1e-6, "minimize_every_iter": True}
    ...
    def test_11_f_min_0(self):
        ...
        res = shgo(
            test1_2.f, test1_2.bounds, n=10, iters=None, options=options, sampling_method="sobol"
        )
    ...
    def test_12_sobol_inf_cons(self):
        ...
        res = shgo(
            test1_2.f, test1_2.bounds, n=1, iters=None, options=options, sampling_method="sobol"
        )
    ...
    def test_20_constrained_args(self):
        ...
        def g2(x):
            return (
                ...
                - 21
                - 1.645
                * np.sqrt(0.28 * x[0] ** 2 + 0.19 * x[1] ** 2 + 20.5 * x[2] ** 2 + 0.62 * x[3] ** 2)
            )  # >=0
    ...
    def test_21_arg_tuple_sobol(self):
        ...
        ref = minimize(fun, np.zeros(1), bounds=bounds, args=(1,), constraints=constraints)
        assert res.success
        assert_allclose(res.fun, ref.fun)
        assert_allclose(res.x, ref.x)
...
class TestShgoFailures:
    def test_1_maxiter(self):
        ...
        options = {"maxiter": 2}
        res = shgo(
            test4_1.f, test4_1.bounds, n=2, iters=None, options=options, sampling_method="sobol"
        )
    ...
    def test_3_2_no_min_pool_simplicial(self):
        ...
        res = shgo(
            test_table.f, test_table.bounds, n=3, options=options, sampling_method="simplicial"
        )
        np.testing.assert_equal(False, res.success)
...
def test_equality_constraints():
    ...
    constraints = {"type": "eq", "fun": faulty}
    res = shgo(
        lambda x: -np.prod(x), bounds=bounds, constraints=constraints, sampling_method="sobol"
    )
...
def test_gh16971():
    ...
    minimizer_kwargs = {"method": "COBYLA", "options": {"rhobeg": 5, "tol": 5e-1, "catol": 0.05}}
