    ...
    def test_workers(self):
        ...
        with MapWrapper(2) as mapper:
            approx = ScalarFunction(
                ex.fun, x0, (), "2-point", ex.hess, None, (-np.inf, np.inf), workers=mapper
            )
            ...
            ex2 = ExScalarFunction()
            approx = ScalarFunction(
                ex.fun, x0, (), "3-point", ex.hess, None, (-np.inf, np.inf), workers=mapper
            )
            approx_series = ScalarFunction(
                ex2.fun,
                x0,
            ...
            approx = ScalarFunction(
                ex.fun, x0, (), ex.grad, "3-point", None, (-np.inf, np.inf), workers=mapper
            )
            approx_series = ScalarFunction(
                ex2.fun,
                x0,
...
class ExVectorialFunction:
    ...
    def fun(self, x):
        self.nfev += 1
        return np.array(
            [2 * (x[0] ** 2 + x[1] ** 2 - 1) - x[0], 4 * (x[0] ** 3 + x[1] ** 2 - 4) - 3 * x[0]],
            dtype=x.dtype,
        )
...
class TestVectorialFunction(TestCase):
    ...
    def test_workers(self):
        ...
        with MapWrapper(2) as mapper:
            approx = VectorFunction(
                ex.fun, x0, "2-point", ex.hess, None, None, (-np.inf, np.inf), False, workers=mapper
            )
            ...
            approx = VectorFunction(
                ex.fun, x0, "3-point", ex.hess, None, None, (-np.inf, np.inf), False, workers=mapper
            )
            approx_series = VectorFunction(
            ...
            ex2.nfev = ex2.njev = ex2.nhev = 0
            approx = VectorFunction(
                ex.fun, x0, ex.jac, "2-point", None, None, (-np.inf, np.inf), False, workers=mapper
            )
            approx_series = VectorFunction(
    ...
    def test_sparse_analytic_jac(self):
        ...
        vf1 = VectorFunction(
            ex.fun, x0, ex.jac, ex.hess, None, None, (-np.inf, np.inf), sparse_jacobian=None
        )
    ...
    def test_sparse_numerical_jac(self):
        ...
        vf1 = VectorFunction(
            ex.fun, x0, "2-point", ex.hess, None, None, (-np.inf, np.inf), sparse_jacobian=None
        )
...
@pytest.mark.skipif(
    platform.python_implementation() == "PyPy", reason="assert_deallocate not available on PyPy"
)
def test_ScalarFunctionNoReferenceCycle():
    ...
    with assert_deallocated(
        lambda: ScalarFunction(ex.fun, x0, (), ex.grad, ex.hess, None, (-np.inf, np.inf))
    ):
        pass
...
def test_VectorFunctionNoReferenceCycle():
    ...
    x0 = [1.0, 0.0]
    with assert_deallocated(
        lambda: VectorFunction(ex.fun, x0, ex.jac, ex.hess, None, None, (-np.inf, np.inf), None)
    ):
        pass
