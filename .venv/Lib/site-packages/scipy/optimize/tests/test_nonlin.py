...
F2.xin = [1, 2, 3, 4, 5, 6]
F2.KNOWN_BAD = {"linearmixing": nonlin.linearmixing, "excitingmixing": nonlin.excitingmixing}
F2.JAC_KSP_BAD = {}
F2.ROOT_JAC_KSP_BAD = {}
...
class TestNonlin:
    ...
    def _check_nonlin_func(self, f, func, f_tol=1e-2):
        ...
        if func == SOLVERS["krylov"]:
            for method in ["gmres", "bicgstab", "cgs", "minres", "tfqmr"]:
                ...
                x = func(
                    f, f.xin, method=method, line_search=None, f_tol=f_tol, maxiter=200, verbose=0
                )
    ...
    def test_tol_norm_called(self, method):
        ...
        nonlin.newton_krylov(
            F, F.xin, method=method, f_tol=1e-2, maxiter=200, verbose=0, tol_norm=local_norm_func
        )
...
class TestLinear:
    ...
    def _check(self, jac, N, maxiter, complex=False, **kw):
        ...
        sol = nonlin.nonlin_solve(
            func, np.zeros(N), jac, maxiter=maxiter, f_tol=1e-6, line_search=None, verbose=0
        )
    ...
    def _check_autojac(self, A, b):
        ...
        sol = nonlin.nonlin_solve(
            func, np.zeros(b.shape[0]), jac, maxiter=2, f_tol=1e-6, line_search=None, verbose=0
        )
        np.testing.assert_allclose(A @ sol, b, atol=1e-6)
        # test jac input as array -- not a function
        sol = nonlin.nonlin_solve(
            func, np.zeros(b.shape[0]), A, maxiter=2, f_tol=1e-6, line_search=None, verbose=0
        )
        np.testing.assert_allclose(A @ sol, b, atol=1e-6)
...
class TestNonlinOldTests:
    ...
    def test_root_anderson(self):
        res = root(
            F, F.xin, method="anderson", options={"nit": 12, "jac_options": {"alpha": 0.03, "M": 5}}
        )
        assert_(nonlin.norm(res.x) < 0.33)

    def test_root_linearmixing(self):
        res = root(
            F, F.xin, method="linearmixing", options={"nit": 60, "jac_options": {"alpha": 0.5}}
        )
    ...
    def test_root_excitingmixing(self):
        res = root(
            F, F.xin, method="excitingmixing", options={"nit": 20, "jac_options": {"alpha": 0.5}}
        )
