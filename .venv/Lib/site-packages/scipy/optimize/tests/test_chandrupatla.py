...
class TestChandrupatlaMinimize:
    ...
    def test_vectorization(self, shape, xp):
        ...
        attrs = ["x", "fun", "success", "status", "nfev", "nit", "xl", "xm", "xr", "fl", "fm", "fr"]
        for attr in attrs:
    ...
    def test_flags(self, xp):
        ...
        ref_flags = xp.asarray(
            [eim._ECONVERGED, eim._ESIGNERR, eim._ECONVERR, eim._EVALUEERR], dtype=xp.int32
        )
    ...
    def test_input_validation(self, xp):
        ...
        with pytest.raises(ValueError, match=message):
            _chandrupatla_minimize(lambda x: [x[0, ...], x[1, ...], x[1, ...]], *bracket)
...
class TestFindRoot:
    ...
    def test_vectorization(self, shape, xp):
        ...
        assert xp.all(
            (res.x[finite] == res.bracket[0][finite]) | (res.x[finite] == res.bracket[1][finite])
        )
        ...
        assert xp.all(
            ...
            == xp.minimum(xp.abs(res.f_bracket[0][finite]), xp.abs(res.f_bracket[1][finite]))
        )
    ...
    def test_convergence(self, xp):
        ...
        xp_assert_less(res1.bracket[1] - res1.bracket[0], xp.full_like(p, xp.asarray(1e-3)))
        kwargs["tolerances"]["xatol"] = 1e-6
        res2 = find_root(self.f, bracket, **kwargs)
        xp_assert_less(res2.bracket[1] - res2.bracket[0], xp.full_like(p, xp.asarray(1e-6)))
        xp_assert_less(res2.bracket[1] - res2.bracket[0], res1.bracket[1] - res1.bracket[0])
    ...
    def test_nit_expected(self, case, xp):
        ...
        xp_assert_close(res.f_x, xp.asarray(f(root), dtype=xp.float64), rtol=1e-8, atol=2e-3)
