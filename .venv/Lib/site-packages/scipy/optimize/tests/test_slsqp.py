...
class TestSLSQP:
    ...
    def test_minimize_unbounded_approximated(self):
        ...
        for jac in jacs:
            res = minimize(
                self.fun, [-1.0, 1.0], args=(-1.0,), jac=jac, method="SLSQP", options=self.opts
            )
    ...
    def test_minimize_unbounded_given(self):
        ...
        res = minimize(
            self.fun, [-1.0, 1.0], args=(-1.0,), jac=self.jac, method="SLSQP", options=self.opts
        )
        assert_(res["success"], res["message"])
        assert_allclose(res.x, [2, 1])
    ...
    def test_minimize_unbounded_combined(self):
        ...
        res = minimize(
            self.fun_and_jac, [-1.0, 1.0], args=(-1.0,), jac=True, method="SLSQP", options=self.opts
        )
        assert_(res["success"], res["message"])
        assert_allclose(res.x, [2, 1])
    ...
    def test_minimize_inequality_given_vector_constraints(self):
        ...
        res = minimize(
            ...
            method="SLSQP",
            args=(-1.0,),
            constraints={"type": "ineq", "fun": self.f_ieqcon2, "jac": self.fprime_ieqcon2},
            options=self.opts,
        )
        assert_(res["success"], res["message"])
        assert_allclose(res.x, [2, 1])
    ...
    def test_unbounded_given(self):
        ...
        res = fmin_slsqp(
            self.fun, [-1.0, 1.0], args=(-1.0,), fprime=self.jac, iprint=0, full_output=1
        )
        x, fx, its, imode, smode = res
        assert_(imode == 0, imode)
        assert_array_almost_equal(x, [2, 1])
    ...
    def test_equality_approximated(self):
        ...
        res = fmin_slsqp(
            self.fun, [-1.0, 1.0], args=(-1.0,), eqcons=[self.f_eqcon], iprint=0, full_output=1
        )
        x, fx, its, imode, smode = res
        assert_(imode == 0, imode)
    ...
    def test_gh1758(self):
        ...
        res = minimize(
            fun, [8, 0.25], method="SLSQP", constraints=[c1, c2], bounds=[(-0.5, 1), (0, 8)]
        )
