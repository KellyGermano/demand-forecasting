...
class SHGO:
    def __init__(
        ...
        if bnderr.any():
            raise ValueError("Error: lb > ub in bounds " f"{', '.join(str(b) for b in bnderr)}.")
        ...
        if (
            self.minimizer_kwargs["method"].lower() in ("slsqp", "cobyla", "cobyqa", "trust-constr")
            and (
    ...
    def finite_ev(self):
        ...
        if self.disp:
            logging.info(f"Sampling evaluations done = {self.n_sampled} " f"/ {self.maxev}")
        if self.n_sampled >= self.maxev:
            self.stop_global = True

    def finite_time(self):
        if self.disp:
            logging.info(f"Time elapsed = {time.time() - self.init} " f"/ {self.maxtime}")
    ...
    def finite_homology_growth(self):
        ...
        if self.disp:
            logging.info(
                f"Current homology growth = {self.hgrd} " f" (minimum growth = {self.minhgrd})"
            )
        return self.stop_global
    ...
    def sampling_custom(self, n, dim):
        ...
        for i in range(len(self.bounds)):
            self.C[:, i] = (
                self.C[:, i] * (self.bounds[i][1] - self.bounds[i][0]) + self.bounds[i][0]
            )
        return self.C
