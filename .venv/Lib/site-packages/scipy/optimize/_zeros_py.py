...
def results_c(full_output, r, method):
    if full_output:
        ...
        results = RootResults(
            root=x, iterations=iterations, function_calls=funcalls, flag=flag, method=method
        )
        return x, results
    else:
        return r
    ...
    if fprime is not None:
        ...
        for itr in range(maxiter):
            ...
            if fder == 0:
                ...
                if disp:
                    msg += f" Failed to converge after {itr + 1} iterations," f" value is {p0}."
        ...
        for itr in range(maxiter):
            if q1 == q0:
                if p1 != p0:
                    ...
                    if disp:
                        msg += f" Failed to converge after {itr + 1} iterations," f" value is {p1}."
                        raise RuntimeError(msg)
                    warnings.warn(msg, RuntimeWarning, stacklevel=2)
                p = (p1 + p0) / 2.0
...
def _notclose(fs, rtol=_rtol, atol=_xtol):
    ...
    notclosefvals = (
        ...
        and not any(
            any(np.isclose(_f, fs[i + 1 :], rtol=rtol, atol=atol)) for i, _f in enumerate(fs[:-1])
        )
    )
...
class TOMS748Solver:
    ...
    def start(self, f, a, b, args=()):
        ...
        if np.sign(fb) * np.sign(fa) > 0:
            raise ValueError(
                "f(a) and f(b) must have different signs, but " f"f({a:e})={fa:e}, f({b:e})={fb:e} "
            )
        self.fab[:] = [fa, fb]
