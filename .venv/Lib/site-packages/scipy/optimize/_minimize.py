...
def minimize(
    ...
    **Custom minimizers**
    ...
    # - hessp
    if (
        meth not in ("newton-cg", "trust-ncg", "trust-constr", "trust-krylov", "_custom")
        and hessp is not None
    ):
        warn(
            f"Method {method} does not use Hessian-vector product" " information (hessp).",
            RuntimeWarning,
            stacklevel=2,
        )
    # - constraints or bounds
    if meth not in ("cobyla", "cobyqa", "slsqp", "trust-constr", "_custom") and np.any(constraints):
        warn(f"Method {method} cannot handle constraints.", RuntimeWarning, stacklevel=2)
    if (
        meth
        not in (
            "nelder-mead",
    ...
    if meth in ("l-bfgs-b", "tnc", "cobyla", "cobyqa", "slsqp") and options.get(
        ...
        warn(
            f"Method {method} does not support the return_all option.", RuntimeWarning, stacklevel=2
        )
            ...
            if remove_vars:
                ...
                if callable(callback):
                    ...
                    if set(sig.parameters) == {"intermediate_result"}:
                        ...
                        print(callback)
                        callback = _Patch_Callback_Equal_Variables(callback, i_fixed, x_fixed)
                    else:
                        # callback(x)
    ...
    if meth == "nelder-mead":
        ...
        res = _minimize_trustregion_constr(
            fun, x0, args, jac, hess, hessp, bounds, constraints, callback=callback, **options
        )
    elif meth == "dogleg":
