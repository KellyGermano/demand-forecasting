...
def _cholesky(a, lower=False, overwrite_a=False, clean=True, check_finite=True):
    ...
    if a1.shape[0] != a1.shape[1]:
        raise ValueError("Input array is expected to be square but has " f"the shape: {a1.shape}.")
    ...
    if info < 0:
        raise ValueError(
            f"LAPACK reported an illegal value in {-info}-th argument " f'on entry to "POTRF".'
        )
    return c, lower
...
def cho_solve_banded(cb_and_lower, b, overwrite_b=False, check_finite=True):
    ...
    return _cho_solve_banded(cb, b, lower, overwrite_b=overwrite_b, check_finite=check_finite)
