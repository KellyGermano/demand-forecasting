...
def cossin(X, p=None, q=None, separate=False, swap_sign=False, compute_u=True, compute_vh=True):
    ...
    if p or q:
        ...
        if not np.equal(*X.shape[-2:]):
            raise ValueError(
                "Cosine Sine decomposition only supports square" f" matrices, got {X.shape[-2:]}"
            )
        m = X.shape[-2]
        if p >= m or p <= 0:
        ...
        if len(X) != 4:
            raise ValueError(
                "When p and q are None, exactly four arrays" f" should be in X, got {len(X)}"
            )
...
def _cossin(x11, x12, x21, x22, separate, swap_sign, compute_u, compute_vh):
    ...
    if x12.shape != (p, mmq):
        raise ValueError(f"Invalid x12 dimensions: desired {(p, mmq)}, " f"got {x12.shape}")

    if x21.shape != (mmp, q):
        raise ValueError(f"Invalid x21 dimensions: desired {(mmp, q)}, " f"got {x21.shape}")
    ...
    *_, theta, u1, u2, v1h, v2h, info = csd(
    ...
    if info < 0:
        raise ValueError(f"illegal value in argument {-info} " f"of internal {method_name}")
    if info > 0:
