...
def solve_continuous_are(a, b, q, r, e=None, s=None, balanced=True):
    ...
    _, _, _, _, _, u = ordqz(
        H, J, sort="lhp", overwrite_a=True, overwrite_b=True, check_finite=False, output=out_str
    )
...
def solve_discrete_are(a, b, q, r, e=None, s=None, balanced=True):
    ...
    _, _, _, _, _, u = ordqz(
        H, J, sort="iuc", overwrite_a=True, overwrite_b=True, check_finite=False, output=out_str
    )
    ...
    if norm(u_sym, 1) > sym_threshold:
        raise LinAlgError(
            "The associated symplectic pencil has eigenvalues " "too close to the unit circle"
        )
