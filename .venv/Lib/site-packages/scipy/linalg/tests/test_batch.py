...
class TestBatch:
        ...
        *,
        ...
        for i in range(batch_shape[0]):
            for j in range(batch_shape[1]):
                ...
                ref = (np.asarray(ref),) if n_out == 1 else tuple(np.asarray(refk) for refk in ref)
    ...
    def test_eig_banded(self, eigvals_only, dtype):
        ...
        self.batch_test(linalg.eig_banded, A, n_out=n_out, kwargs=dict(eigvals_only=eigvals_only))
    ...
    def test_expm_frechet(self, compute_expm, dtype):
        ...
        self.batch_test(
            linalg.expm_frechet, (A, E), n_out=n_out, kwargs=dict(compute_expm=compute_expm)
        )
    ...
    @pytest.mark.parametrize("fun", [linalg.solve_continuous_are, linalg.solve_discrete_are])
    @pytest.mark.parametrize("dtype", floating)
    ...
    def test_block_diag(self, dtype):
        ...
        ref = self.batch_test(linalg.block_diag, (a2, b2, c2), check_kwargs=False, broadcast=False)
    ...
    def test_clarkson_woodruff_transform(self, dtype):
        ...
        A = get_random((5, 3, 4, 6), dtype=dtype, rng=rng)
        self.batch_test(
            linalg.clarkson_woodruff_transform, A, kwargs=dict(sketch_size=3, rng=311224)
        )
