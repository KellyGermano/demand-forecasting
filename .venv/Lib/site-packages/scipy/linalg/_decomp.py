...
def eig(
    ...
    _check_info(
        info,
        "eig algorithm (geev)",
        positive="did not converge (only eigenvalues " "with order >= %d have converged)",
    )
...
def eigh(
    ...
    *,
    ...
    if info == 0:
        ...
        if info < -1:
            raise LinAlgError(
                f"Illegal value in argument {-info} of internal " f"{drv.typecode + pfx + driver}"
            )
        elif info > n:
            raise LinAlgError(
...
@_apply_over_batch(("a_band", 2))
def eigvals_banded(
    a_band, lower=False, overwrite_a_band=False, select="a", select_range=None, check_finite=True
):
    """
    Solve real symmetric or complex Hermitian band matrix eigenvalue problem.
...
def eigvalsh_tridiagonal(
    d, e, select="a", select_range=None, check_finite=True, tol=0.0, lapack_driver="auto"
):
    """
...
def eigh_tridiagonal(
    ...
    if lapack_driver not in drivers:
        raise ValueError(f"lapack_driver must be one of {drivers}, " f"got {lapack_driver}")
    ...
    if lapack_driver == "sterf":
        ...
        m, w, v, info = func(
            d, e_, select, vl, vu, il, iu, compute_v=compute_v, lwork=lwork, liwork=liwork
        )
    ...
    if eigvals_only:
        ...
        if lapack_driver == "stebz":
            ...
            _check_info(
                info, "stein (eigh_tridiagonal)", positive="%d eigenvectors failed to converge"
            )
