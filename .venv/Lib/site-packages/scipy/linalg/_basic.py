...
from ._cythonized_array_utils import find_det_from_lu, bandwidth, issymmetric, ishermitian
...
def _solve_check(n, info, lamch=None, rcond=None):
    ...
    if not (rcond >= E):  # `rcond < E` doesn't handle NaN
        warn(
            f"Ill-conditioned matrix (rcond={rcond:.6g}): " "result may not be accurate.",
            LinAlgWarning,
            stacklevel=3,
        )
...
def solve(
    ...
    if assume_a == "diagonal":
        ...
        a2, n_below, n_above = (a1.T, n_above, n_below) if transposed else (a1, n_below, n_above)
    ...
    # Hermitian case 'hesv'
        ...
        lu, ipvt, x, info = hesv(
            a1, b1, lwork=lwork, lower=lower, overwrite_a=overwrite_a, overwrite_b=overwrite_b
        )
        _solve_check(n, info)
    ...
    # Triangular case
        ...
        x, info = _solve_triangular(a1, b1, lower=lower, overwrite_b=overwrite_b, trans=transposed)
        _solve_check(n, info)
...
def _solve_toeplitz(c, r, b, check_finite):
    ...
    if b.ndim == 1:
        ...
        x = np.column_stack(
            [levinson(vals, np.ascontiguousarray(b[:, i]))[0] for i in range(b.shape[1])]
        )
        x = x.reshape(*b_shape)
...
def det(a, overwrite_a=False, check_finite=True):
    ...
    if a1.shape[-1] != a1.shape[-2]:
        raise ValueError(
            "Last 2 dimensions of the array must be square" f" but received shape {a1.shape}."
        )
    ...
    if driver in ("gelss", "gelsd"):
        if driver == "gelss":
            ...
            if real_data:
                ...
                x, s, rank, info = lapack_func(a1, b1, lwork, rwork, iwork, cond, False, False)
        if info > 0:
