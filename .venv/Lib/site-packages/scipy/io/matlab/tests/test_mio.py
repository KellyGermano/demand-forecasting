...
case_table4 = [
    {"name": "double", "classes": {"testdouble": "double"}, "expected": {"testdouble": theta}}
]
case_table4.append(
    {
        "name": "string",
        "classes": {"teststring": "char"},
        "expected": {"teststring": array(['"Do nine men interpret?" "Nine men," I nod.'])},
    }
)
case_table4.append(
    {
...
CA = mlarr(
    ([], mlarr([1]), mlarr([[1, 2]]), mlarr([[1, 2, 3]])),  # tuple for object array creation
    dtype=object,
).reshape(1, -1)
...
CAE = mlarr(
    (mlarr(1), mlarr(2), mlarr([]), mlarr([]), mlarr(3)),  # tuple for object array creation
    dtype=object,
).reshape(1, -1)
...
case_table5.append(
    {"name": "emptycell", "classes": {"testemptycell": "cell"}, "expected": {"testemptycell": CAE}}
)
case_table5.append(
    {
        "name": "stringarray",
...
case_table5.append(
    {
        ...
        "expected": {"test3dmatrix": np.transpose(np.reshape(list(range(1, 25)), (4, 3, 2)))},
    }
)
...
case_table5.append(
    {"name": "struct", "classes": {"teststruct": "struct"}, "expected": {"teststruct": st1}}
)
...
case_table5.append(
    {"name": "structarr", "classes": {"teststructarr": "struct"}, "expected": {"teststructarr": a}}
)
ODT = np.dtype(
    [(n, object) for n in ["expr", "inputExpr", "args", "isEmpty", "numArgs", "version"]]
)
...
m0["version"] = mlarr(1)
case_table5.append(
    {"name": "object", "classes": {"testobject": "object"}, "expected": {"testobject": MO}}
)
...
def test_4_and_long_field_names_incompatible():
    ...
    assert_raises(
        ValueError, savemat, BytesIO(), {"my_struct": my_struct}, format="4", long_field_names=True
    )
...
def test_long_field_names_in_struct():
    ...
    #
    assert_raises(
        ValueError, savemat, BytesIO(), {"longstruct": cell}, format="5", long_field_names=False
    )
