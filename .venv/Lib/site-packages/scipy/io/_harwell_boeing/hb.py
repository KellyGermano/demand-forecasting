...
class HBInfo:
    ...
    def from_data(cls, m, title="Default title", key="0", mxtype=None, fmt=None):
        ...
        if mxtype is None:
            ...
            if values.dtype.kind in np.typecodes["AllInteger"]:
                ...
                raise NotImplementedError(f"type {values.dtype} for values not implemented")
    ...
    def from_file(cls, fid):
        ...
        if not len(line) > 72:
            raise ValueError("Expected at least 72 characters for first line, " f"got: \n{line}")
        title = line[:72]
        key = line[72:]
        ...
        if not len(line.rstrip()) >= 56:
            raise ValueError("Expected at least 56 characters for second line, " f"got: \n{line}")
        ...
        line = fid.readline().strip("\n")
        if not len(line) >= 70:
            raise ValueError(f"Expected at least 72 character for third line, " f"got:\n{line}")
        ...
        if mxtype.value_type not in ["real", "integer"]:
            raise ValueError(
                "Only real or integer matrices supported for " f"now (detected {mxtype})"
            )
        if not mxtype.structure == "unsymmetric":
            raise ValueError("Only unsymmetric matrices supported for " f"now (detected {mxtype})")
    ...
    def __init__(
        ...
        if key is None:
            key = "|No Key"
        if len(key) > 8:
            warnings.warn(f"key is > 8 characters (key is {key})", LineOverflow, stacklevel=3)
        self.title = title
        self.key = key
        ...
        if not isinstance(pointer_format, IntFormat):
            raise ValueError("Expected int format for pointer format, got " f"{pointer_format}")

        indices_format = parser.parse(indices_format_str)
        if not isinstance(indices_format, IntFormat):
            raise ValueError("Expected int format for indices format, got " f"{indices_format}")
        ...
        if isinstance(values_format, ExpFormat):
            if mxtype.value_type not in ["real", "complex"]:
                raise ValueError(
                    f"Inconsistency between matrix type {mxtype} and " f"value type {values_format}"
                )
