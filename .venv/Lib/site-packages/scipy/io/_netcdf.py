...
class netcdf_file:
        ...
        if not var.isrec:
            ...
            for rec in var.data:
                ...
                if not rec.shape and (
                    rec.dtype.byteorder == "<" or (rec.dtype.byteorder == "=" and LITTLE_ENDIAN)
                ):
                    rec = rec.byteswap()
    ...
    def _write_att_values(self, values):
        ...
        if not values.shape and (
            values.dtype.byteorder == "<" or (values.dtype.byteorder == "=" and LITTLE_ENDIAN)
        ):
    ...
    def _read_var_array(self):
        ...
        for var in range(count):
            (name, dimensions, shape, attributes, typecode, size, dtype_, begin_, vsize) = (
                self._read_var()
            )
            ...
            # Add variable.
            self.variables[name] = netcdf_variable(
                data, typecode, size, shape, dimensions, attributes, maskandscale=self.maskandscale
            )
...
class netcdf_variable:
    ...
    def __init__(
        self, data, typecode, size, shape, dimensions, attributes=None, maskandscale=False
    ):
        self.data = data
    ...
    def __setitem__(self, index, data):
        if self.maskandscale:
            ...
            data = (data - self._attributes.get("add_offset", 0.0)) / self._attributes.get(
                "scale_factor", 1.0
            )
