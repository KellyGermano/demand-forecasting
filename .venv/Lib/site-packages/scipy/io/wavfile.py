...
def _read_data_chunk(
    ...
    if format_tag == WAVE_FORMAT.PCM:
        if 1 <= bit_depth <= 8:
            ...
            raise ValueError(
                "Unsupported bit depth: the WAV file " f"has {bit_depth}-bit integer data."
            )
        ...
        if bit_depth in {32, 64}:
            ...
            raise ValueError(
                "Unsupported bit depth: the WAV file " f"has {bit_depth}-bit floating-point data."
            )
    else:
        _raise_bad_format(format_tag)
    ...
    try:
        ...
        while fid.tell() < file_size:
            ...
            if chunk_id == b"fmt ":
                ...
                warnings.warn(
                    "Chunk (non-data) not understood, skipping it.", WavFileWarning, stacklevel=2
                )
                _skip_unknown_chunk(fid, is_big_endian)
    finally:
