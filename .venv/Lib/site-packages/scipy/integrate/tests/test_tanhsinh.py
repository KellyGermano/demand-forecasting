...
class TestTanhSinh:
    ...
    @pytest.mark.skip_xp_backends(np_only=True, reason="Cumbersome to convert everything.")
    ...
    @pytest.mark.skip_xp_backends(np_only=True, reason="Distributions aren't xp-compatible.")
    ...
    def test_preserve_shape(self, xp):
        ...
        def f(x, xp):
            return xp.stack(
                [xp.stack([x, xp.sin(10 * x)]), xp.stack([xp.cos(30 * x), x * xp.sin(100 * x)])]
            )

        ref = quad_vec(lambda x: f(x, np), 0, 1)
        res = _tanhsinh(lambda x: f(x, xp), xp.asarray(0), xp.asarray(1), preserve_shape=True)
    ...
    def test_log(self, rtol, xp):
        ...
        with np.errstate(all="ignore"):
            xp_assert_close(xp.exp(res.integral), ref.integral, **test_tols, check_dtype=False)
            xp_assert_close(xp.exp(res.error), ref.error, **test_tols, check_dtype=False)
        assert res.nfev == ref.nfev
...
class TestNSum:
    ...
    def test_integral(self, maxterms, xp):
        ...
        rtol = 1e-12
        res = nsum(f, a, b_original, step=step, maxterms=maxterms, tolerances=dict(rtol=rtol))
    ...
    def test_vectorization(self, shape, xp):
        ...
        res = nsum(f, xp.asarray(a), xp.asarray(b), maxterms=1000, args=(xp.asarray(p),))
