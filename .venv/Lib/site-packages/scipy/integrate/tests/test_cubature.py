...
def _eval_indefinite_integral(F, a, b, xp):
    ...
    for ind in itertools.product(range(2), repeat=ndim):
        selected_points = xp.asarray([float(points[i, j]) for i, j in zip(ind, range(ndim))])
...
def genz_malik_1980_f_5(x, alphas, betas, xp):
    ...
    return xp.exp(-xp.sum(alphas_reshaped**2 * (x_reshaped - betas_reshaped) ** 2, axis=-1))
...
def genz_malik_1980_f_5_exact(a, b, alphas, betas, xp):
    ...
    return (
        ...
        * (math.pi ** (ndim / 2))
        * xp.prod(
            scipy.special.erf(alphas * (betas - a)) + scipy.special.erf(alphas * (b - betas)),
            axis=-1,
        )
    )
...
def f_gaussian_exact(a, b, alphas, xp):
    ...
    return (math.sqrt(math.pi) ** ndim) / (2**semi_infinite_count * xp.prod(alphas, axis=-1))
...
class TestCubatureProblems:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (genz_malik_1980_f_3, genz_malik_1980_f_3_exact, genz_malik_1980_f_3_random_args),
            (genz_malik_1980_f_4, genz_malik_1980_f_4_exact, genz_malik_1980_f_4_random_args),
            (
                genz_malik_1980_f_5,
                genz_malik_1980_f_5_exact,
...
class TestRulesQuadrature:
    ...
    def test_base_1d_quadratures_error_from_difference(self, rule_pair, rule_pair_args, xp):
        n = xp.arange(5, dtype=xp.float64)
        a = xp.asarray([0], dtype=xp.float64)
        b = xp.asarray([2], dtype=xp.float64)
