...
def _cpu_get_cpuinfo_freq():
    ...
    with open_binary(f"{get_procfs_path()}/cpuinfo") as f:
        return [float(line.split(b":", 1)[1]) for line in f if line.lower().startswith(b"cpu mhz")]
        ...
        with open_text(file) as f:
            ...
            for lineno, line in enumerate(f, 1):
                try:
                    ...
                    msg = f"error while parsing {file}; malformed line" f" {lineno} {line!r}"
...
def disk_io_counters(perdisk=False):
    ...
    if os.path.exists(f"{get_procfs_path()}/diskstats"):
        ...
        msg = f"{get_procfs_path()}/diskstats nor /sys/block are available on" " this system"
        raise NotImplementedError(msg)
...
def sensors_temperatures():
    ...
    if not basenames:
        ...
        for base in basenames:
            ...
            trip_points = {"_".join(os.path.basename(p).split("_")[0:3]) for p in trip_paths}
            critical = None
            high = None
...
def sensors_battery():
    ...
    bats = [
        x for x in os.listdir(POWER_SUPPLY_PATH) if x.startswith("BAT") or "battery" in x.lower()
    ]
    if not bats:
        return None
...
class Process:
    ...
    @wrap_exceptions
    def exe(self):
        return self._readlink(f"{self._procfs_path}/{self.pid}/exe", fallback="")
    ...
    @wrap_exceptions
    def cwd(self):
        return self._readlink(f"{self._procfs_path}/{self.pid}/cwd", fallback="")
    ...
    if HAS_CPU_AFFINITY:
        ...
        def _get_eligible_cpus(self, _re=re.compile(rb"Cpus_allowed_list:\t(\d+)-(\d+)")):
                ...
                if isinstance(err, ValueError) or err.errno == errno.EINVAL:
                    ...
                    for cpu in cpus:
                        if cpu not in all_cpus:
                            msg = f"invalid CPU {cpu!r}; choose between" f" {eligible_cpus!r}"
                            raise ValueError(msg) from None
        ...
        def rlimit(self, resource_, limits=None):
            ...
            try:
                if limits is None:
                    ...
                    # set
                    if len(limits) != 2:
                        msg = "second argument must be a (soft, hard) " f"tuple, got {limits!r}"
                        raise ValueError(msg)
