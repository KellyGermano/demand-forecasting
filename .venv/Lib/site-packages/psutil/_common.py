...
def broadcast_addr(addr):
    ...
    if addr.family == socket.AF_INET:
        return str(
            ipaddress.IPv4Network(f"{addr.address}/{addr.netmask}", strict=False).broadcast_address
        )
    if addr.family == socket.AF_INET6:
        return str(
            ipaddress.IPv6Network(f"{addr.address}/{addr.netmask}", strict=False).broadcast_address
        )
...
def deprecated_method(replacement):
    ...
    def outer(fun):
        msg = f"{fun.__name__}() is deprecated and will be removed; use" f" {replacement}() instead"
...
def debug(msg):
    ...
    if PSUTIL_DEBUG:
        ...
        fname, lineno, _, _lines, _index = inspect.getframeinfo(inspect.currentframe().f_back)
