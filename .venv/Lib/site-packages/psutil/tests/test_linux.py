    ...
    def test_missing_sin_sout(self):
        with mock.patch("psutil._common.open", create=True) as m:
            with warnings.catch_warnings(record=True) as ws:
                ...
                assert len(ws) == 1
                w = ws[0]
                assert "'sin' and 'sout' swap memory stats couldn't be determined" in str(w.message)
                assert ret.sin == 0
                assert ret.sout == 0
...
class TestSystemCPUCountCores(PsutilTestCase):
    ...
    @pytest.mark.skipif(platform.machine() not in {"x86_64", "i686"}, reason="x86_64/i686 only")
...
class TestSystemNetIfStats(PsutilTestCase):
    @pytest.mark.skipif(not shutil.which("ifconfig"), reason="ifconfig utility not available")
...
class TestSystemNetIOCounters(PsutilTestCase):
    ...
    def test_against_ifconfig(self):
        def ifconfig(nic):
            ...
            ret["bytes_recv"] = int(re.findall(r"RX (?:packets \d+ +)?bytes[: ](\d+)", out)[0])
            ret["bytes_sent"] = int(re.findall(r"TX (?:packets \d+ +)?bytes[: ](\d+)", out)[0])
            return ret
...
class TestRootFsDeviceFinder(PsutilTestCase):
    ...
    @pytest.mark.skipif(not shutil.which("findmnt"), reason="findmnt utility not available")
    @pytest.mark.skipif(GITHUB_ACTIONS, reason="unsupported on GITHUB_ACTIONS")
...
class TestSensorsBattery(PsutilTestCase):
    ...
    def test_emulate_no_power(self):
        ...
        with mock_open_exception("/sys/class/power_supply/AC/online", FileNotFoundError):
            with mock_open_exception("/sys/class/power_supply/AC0/online", FileNotFoundError):
                with mock_open_exception(
...
class TestProcess(PsutilTestCase):
    ...
    def test_open_files_enametoolong(self):
        ...
        with open(self.get_testfn(), "w"):
            ...
            with mock.patch(patch_point, side_effect=OSError(errno.ENAMETOOLONG, "")) as m:
    ...
    def test_rlimit_zombie(self):
        ...
        with mock.patch("resource.prlimit", side_effect=OSError(errno.ENOSYS, "")) as m1:
            with mock.patch("psutil._pslinux.Process._is_zombie", return_value=True) as m2:
                p = psutil.Process()
                p.name()
