...
class BSDTestCase(PsutilTestCase):
    ...
    @pytest.mark.skipif(not shutil.which("ifconfig"), reason="ifconfig cmd not available")
...
class FreeBSDSystemTestCase(PsutilTestCase):
    ...
    def test_cpu_stats_ctx_switches(self):
        assert abs(psutil.cpu_stats().ctx_switches - sysctl("vm.stats.sys.v_swtch")) < 1000

    def test_cpu_stats_interrupts(self):
        assert abs(psutil.cpu_stats().interrupts - sysctl("vm.stats.sys.v_intr")) < 1000

    def test_cpu_stats_soft_interrupts(self):
        assert abs(psutil.cpu_stats().soft_interrupts - sysctl("vm.stats.sys.v_soft")) < 1000

    @retry_on_failure()
    def test_cpu_stats_syscalls(self):
        # pretty high tolerance but it looks like it's OK.
        assert abs(psutil.cpu_stats().syscalls - sysctl("vm.stats.sys.v_syscall")) < 200000
    ...
    def test_sensors_temperatures_against_sysctl(self):
        ...
        for cpu in range(num_cpus):
            ...
            assert abs(psutil.sensors_temperatures()["coretemp"][cpu].current - sysctl_result) < 10
...
class NetBSDTestCase(PsutilTestCase):
    ...
    def test_vmem_free(self):
        assert (
            abs(psutil.virtual_memory().free - self.parse_meminfo("MemFree:")) < TOLERANCE_SYS_MEM
        )
    ...
    def test_vmem_cached(self):
        assert (
            abs(psutil.virtual_memory().cached - self.parse_meminfo("Cached:")) < TOLERANCE_SYS_MEM
        )
    ...
    def test_swapmem_total(self):
        assert (
            abs(psutil.swap_memory().total - self.parse_meminfo("SwapTotal:")) < TOLERANCE_SYS_MEM
        )

    def test_swapmem_free(self):
        assert abs(psutil.swap_memory().free - self.parse_meminfo("SwapFree:")) < TOLERANCE_SYS_MEM
