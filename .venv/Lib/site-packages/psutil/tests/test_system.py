    ...
    def test_virtual_memory(self):
        ...
        for name in mem._fields:
            ...
            if name != "total":
                ...
                if value > mem.total:
                    raise pytest.fail(f"{name!r} > total (total={mem.total}, {name}={value})")
...
class TestCpuAPIs(PsutilTestCase):
    ...
    @pytest.mark.skipif((CI_TESTING and OPENBSD) or MACOS, reason="unreliable on OPENBSD + CI")
    @retry_on_failure(30)
    ...
    def test_per_cpu_times_percent_negative(self):
        ...
        zero_times = [
            x._make([0 for x in range(len(x._fields))]) for x in psutil.cpu_times(percpu=True)
        ]
