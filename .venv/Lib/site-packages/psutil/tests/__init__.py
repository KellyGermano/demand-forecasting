...
class TestMemoryLeak(PsutilTestCase):
    ...
    def _check_fds(self, fun):
        ...
        if diff < 0:
            msg = (
                f"negative diff {diff!r} (gc probably collected a" " resource from a previous test)"
            )
...
def print_sysinfo():
    ...
    # constants
    constants = sorted([k for k, v in globals().items() if k.isupper() and v is True])
    info["constants"] = "\n                  ".join(constants)
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if only_if is not None:
                    if not only_if:
                        raise
                msg = f"{fun.__name__!r} was skipped because it raised" " NotImplementedError"
