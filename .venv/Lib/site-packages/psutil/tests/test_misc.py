...
class TestSpecialMethods(PsutilTestCase):
    ...
    def test_no_such_process__str__(self):
        ...
        assert (
            str(psutil.NoSuchProcess(321, name="name", msg="msg")) == "msg (pid=321, name='name')"
        )

    def test_zombie_process__repr__(self):
        assert (
            repr(psutil.ZombieProcess(321)) == 'psutil.ZombieProcess(pid=321, msg="PID still '
    ...
    def test_zombie_process__str__(self):
        assert str(psutil.ZombieProcess(321)) == "PID still exists but it's a zombie (pid=321)"
        assert (
    ...
    def test_access_denied__str__(self):
        ...
        assert str(psutil.AccessDenied(321, name="name", msg="msg")) == "msg (pid=321, name='name')"
...
class TestMisc(PsutilTestCase):
                ...
                try:
                    ...
                    if name not in psutil.__all__:
                        ...
                        if fun.__doc__ is not None and "deprecated" not in fun.__doc__.lower():
                            raise pytest.fail(f"{name!r} not in psutil.__all__")
    ...
    def test_serialization(self):
        ...
        b = pickle.loads(pickle.dumps(psutil.NoSuchProcess(pid=4567, name="name", msg="msg")))
        ...
        b = pickle.loads(
            pickle.dumps(psutil.ZombieProcess(pid=4567, name="name", ppid=42, msg="msg"))
        )
        ...
        b = pickle.loads(pickle.dumps(psutil.AccessDenied(pid=123, name="name", msg="msg")))
        assert isinstance(b, psutil.AccessDenied)
        assert b.pid == 123
        assert b.name == "name"
        assert b.msg == "msg"

        b = pickle.loads(pickle.dumps(psutil.TimeoutExpired(seconds=33, pid=4567, name="name")))
    ...
    def test_ad_on_process_creation(self):
        ...
        with mock.patch.object(psutil.Process, "_get_ident", side_effect=ValueError) as meth:
...
class TestWrapNumbers(PsutilTestCase):
    ...
    def test_cache_call_twice(self):
        ...
        cache = wrap_numbers.cache_info()
        assert cache[0] == {"disk_io": input}
        assert cache[1] == {"disk_io": {("disk1", 0): 0, ("disk1", 1): 0, ("disk1", 2): 0}}
        assert cache[2] == {"disk_io": {}}
    ...
    def test_cache_wrap(self):
        ...
        cache = wrap_numbers.cache_info()
        assert cache[0] == {"disk_io": input}
        assert cache[1] == {"disk_io": {("disk1", 0): 0, ("disk1", 1): 0, ("disk1", 2): 100}}
        ...
        cache = wrap_numbers.cache_info()
        assert cache[0] == {"disk_io": input}
        assert cache[1] == {"disk_io": {("disk1", 0): 0, ("disk1", 1): 0, ("disk1", 2): 190}}
        assert cache[2] == {"disk_io": {"disk1": {("disk1", 2)}}}
