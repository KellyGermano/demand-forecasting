...
class TestSystemAPIs(WindowsTestCase):
    def test_nic_names(self):
        ...
        for nic in nics:
            if "pseudo-interface" in nic.replace(" ", "-").lower():
                continue
            if nic not in out:
                raise pytest.fail(f"{nic!r} nic wasn't found in 'ipconfig /all' output")
    ...
    def test_free_phymem(self):
        ...
        assert abs(int(w.AvailableBytes) - psutil.virtual_memory().free) < TOLERANCE_SYS_MEM

    def test_total_swapmem(self):
        w = wmi.WMI().Win32_PerfRawData_PerfOS_Memory()[0]
        assert int(w.CommitLimit) - psutil.virtual_memory().total == psutil.swap_memory().total
            ...
            for wmi_part in wmi_parts:
                if ps_part.device.replace("\\", "") == wmi_part.DeviceID:
                    ...
                    if abs(usage.free - wmi_free) > 10 * 1024 * 1024:
                        raise pytest.fail(f"psutil={usage.free}, wmi={wmi_free}")
                    break
            else:
    ...
    def test_convert_dos_path_drive(self):
        ...
        with mock.patch("psutil._pswindows.cext.QueryDosDevice", side_effect=devices.get) as m:
...
class TestSensorsBattery(WindowsTestCase):
    ...
    def test_emulate_power_connected(self):
        with mock.patch("psutil._pswindows.cext.sensors_battery", return_value=(1, 0, 0, 0)) as m:
...
class TestProcess(WindowsTestCase):
    ...
    def test_memory_info(self):
        ...
        assert sys_value["QuotaPeakNonPagedPoolUsage"] == psutil_value.peak_nonpaged_pool
