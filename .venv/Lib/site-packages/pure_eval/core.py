...
class Evaluator:
    ...
    def _handle_call(self, node):
        ...
        if (
            ...
            args = [of_standard_types(arg, check_dict_values=False, deep=False) for arg in args]
            try:
                return func(*args)
        ...
        if len(args) == 1:
            ...
            if is_any(func, sorted, min, max, hash, set, dict, ascii, str, repr, frozenset):
    ...
    def _handle_subscript(self, node):
        ...
        if isinstance(index, ast.Slice):
            index = slice(
                *[None if p is None else self[p] for p in [index.lower, index.upper, index.step]]
            )
    ...
    def interesting_expressions_grouped(self, root: ast.AST) -> List[Tuple[List[ast.expr], Any]]:
        ...
        return group_expressions(
            pair for pair in self.find_expressions(root) if is_expression_interesting(*pair)
        )
