...
class JupyterPTCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):
        ...
        content = self.jup_completer.complete_request(
            code=document.text, cursor_pos=document.cursor_position
        )
        ...
        if "_jupyter_types_experimental" in meta:
            try:
                new_meta = {}
                for c, m in zip(content["matches"], meta["_jupyter_types_experimental"]):
                    new_meta[c] = m["type"]
                meta = new_meta
            except Exception:
                pass
...
class ZMQTerminalInteractiveShell(SingletonConfigurable):
    ...
    highlighting_style = Unicode(
        "", config=True, help="The name of a Pygments style to use for syntax highlighting"
    )
    ...
    async def interact(self, loop=None, display_banner=None):
        while self.keep_running:
            ...
            try:
                ...
                if (not self.confirm_exit) or ask_yes_no(
    ...
    def handle_input_request(self, msg_id, timeout=0.1):
        ...
        if msg_id == req["parent_header"].get("msg_id"):
            ...
            if not (
                run_sync(self.client.stdin_channel.msg_ready)()
