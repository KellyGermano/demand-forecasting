...
def LocateWebServerPath(description):
    ...
    for site in iis:
        ...
        site_attributes = [
            getattr(site, attr, "").lower().strip() for attr in ("Name", "ServerComment")
        ]
...
def build_usage(handler_map: Mapping[str, object]) -> str:
    ...
    lines = [" %-10s: %s" % (arg, handler.__doc__) for arg, handler in handler_map.items()]
...
def HandleCommandLine(
    ...
    parser.add_option(
        ...
        "--server",
        action="store",
        help="Specifies the IIS server to install/uninstall on." f" Default is '{_IIS_OBJECT}/1'",
    )
